...
def main() -> None:
    ...
    eg = {row["verification_id"]: row for row in data["source_verifications"]}["MF-EG745-BYTE-PARITY-20260904"]
    ...
    for token in ("sbu001@monterecco.com", "91 3352129", "c/ Fortuny", "DNI", "NIE"):
        require(token.lower() not in public_text.lower(), f"public continuity pages expose forbidden token {token}")
    release_text = LEDGER.read_text(encoding="utf-8") + "\n" + REPORT.read_text(encoding="utf-8") + "\n" + public_text
    require("0d96284231db1d126cf9477dd5f9b837819bde475e89623ab512749396eda4cf" not in release_text, "discarded provisional hash reintroduced")
    require("MF-REF-COLLISION-745-2026" not in release_text, "discarded E.G. 745 collision hypothesis reintroduced")
