...
check(registry.get("counts") == EXPECTED_REGISTRY_COUNTS, "canonical source registry counts do not equal 230/95/79/11/23/22")
...
check({key: digest_identity.get(key) for key in EXPECTED_REGISTRY_COUNTS} == EXPECTED_REGISTRY_COUNTS, "current digest source/static identity registry is not 230/95/79/11/23/22")
...
print(" - canonical source registry: 230 / 95 / 79 / 11 / 23 / 22; prior exact-live snapshot remains historical")
