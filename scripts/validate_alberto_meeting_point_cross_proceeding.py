...
check(registry.get("counts") == EXPECTED_REGISTRY_COUNTS, "canonical source registry counts do not equal 240/95/79/11/23/23")
...
check({key: digest_identity.get(key) for key in EXPECTED_REGISTRY_COUNTS} == EXPECTED_REGISTRY_COUNTS, "current digest source/static identity registry is not 240/95/79/11/23/23")
...
print(" - canonical source registry: 240 / 102 / 79 / 11 / 25 / 23; prior exact-live snapshot remains historical")
