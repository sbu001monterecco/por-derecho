...
check(registry.get("counts") == EXPECTED_REGISTRY_COUNTS, "canonical source registry counts do not equal 228/95/79/11/23/20")
...
check({key: digest_identity.get(key) for key in EXPECTED_REGISTRY_COUNTS} == EXPECTED_REGISTRY_COUNTS, "current digest source/static identity registry is not 228/95/79/11/23/20")
...
print(" - canonical source registry: 228 / 95 / 79 / 11 / 23 / 20; prior exact-live snapshot remains historical")
