#!/usr/bin/env python3
"""Bounded website repairs: source-preserving, reviewable and reversible.

Preserves every narrative character outside explicitly logged attribute changes
and additive documentary fallback. Does not synthesize source events, resolve
pending identities, alter protection, or merge held historical branches.
"""
...
def repair_attributes(path,text):
 ...
 def tag_replace(m):
  ...
  def attr_replace(a):
   ...
   encoded=html.escape(new,quote=True);old=a.group(0);replacement=old[:a.start(3)-a.start()]+encoded+old[a.end(3)-a.start():]
...
def graph_data():
 ...
 assert all(str(x.get('from',x.get('source'))) in ids and str(x.get('to',x.get('target'))) in ids for x in e)
 return d,n,e
...
def documentary(lang):
 ...
 parts=[f'<!-- {MARKER}:BEGIN -->',f'<section id="{MARKER}" class="pd-audit-graph-documentary">',f'<h2>{t("Edición documental de las relaciones","Documentary relationship edition")}</h2>',f'<p>{t("Esta edición reproduce las relaciones y límites del registro existente. La dirección de una flecha no acredita causalidad, conocimiento o culpabilidad. Los filtros de la visualización no ocultan esta edición completa.","This edition reproduces the existing registry relationships and limits. Arrow direction does not establish causation, knowledge or guilt. Visualization filters do not hide this complete edition.")}</p>']
 source_map={x['id']:x for x in d.get('sources',[]) if isinstance(x,dict) and 'id' in x}
 for e in edges:
  fromid=str(e.get('from',e.get('source')));toid=str(e.get('to',e.get('target')));eid=e.get('id','')
  parts.append('<details data-audit-edge="'+html.escape(str(eid),quote=True)+'"><summary>'+html.escape(names[fromid]+' → '+names[toid])+'</summary>')
  parts.append('<p>'+html.escape(str(eid)+' · '+str(e.get('grade',e.get('type','')))+' · '+str(e.get('period',e.get('date',''))))+'</p>')
  ...
  for raw in e.get('sources',e.get('source_ids',[])):
   ...
   value=s.get('url',s.get('path',s.get('route','')));title=field(s,'title',lang) or s.get('id',str(raw))
   if isinstance(value,str) and value:
    if value.startswith(('https://','http://')):href=value
    else:
     p=ROOT/value.lstrip('/').removeprefix('por-derecho/')
     href=('/por-derecho/'+p.relative_to(ROOT).as_posix()) if p.is_file() else None
...
def prepare():
 ...
 # Only inspect tracked public HTML. Do not touch ignored caches or raw source text.
 paths=git('ls-files','*.html').splitlines()
 for path in paths:
  ...
  if path in NO_REWRITE or any(x in path for x in EXCLUDED):
   probe,changes=repair_attributes(path,text)
  ...
  if len(matches)>1:
   for index,m in reversed(list(enumerate(matches[1:],2))):
    replacement=m.group(0).replace('neutralisation','neutralisation-continuity-'+str(index));text=text[:m.start()]+replacement+text[m.end():]
  ...
  if text!=base:
   # Every non-additive change is a logged attribute/ID substitution.
   save(path,text);modified.append({'path':path,'before_sha256':sha(base),'after_sha256':sha(text),'edits':edits})
 # Missing historic stylesheet becomes a narrow compatibility import. Original
 # hrefs need not be broken and the canonical stylesheet stays untouched.
 ...
 for ident in ['PD-SP-P-0138','PD-SP-P-0139','PD-SP-P-0143']:
  ...
  assert 'PENDING' in str(row.get('identity_resolution','')) or 'UNRESOLVED' in str(row.get('status','')) or 'PENDING' in json.dumps(row),'Only unresolved identities may enter this queue'
 ...
 # Refresh graph fingerprints only; unrelated evidential relationships preserved.
