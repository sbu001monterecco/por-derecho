...
def main() -> int:
    try:
        ...
        for key in (
            ...
            require(isinstance(freshness.get(key), (int, float)), f"freshness policy missing {key}")
        ...
        require(isinstance(inventory.get("open_pull_requests"), int), "open-PR observation must be integer")
        require(inventory["open_pull_requests"] >= 0, "open-PR observation cannot be negative")
        require("Observation only" in str(inventory.get("meaning")), "open-PR observational boundary missing")
        ...
        require(routing.get("expected_control_id") == UNITARY_CONTROL_ID, "unitary control expectation drift")
        require(routing.get("expected_status") == "LIVE_VERIFIED", "unitary status expectation drift")
        require("Neither layer substitutes" in str(routing.get("rule")), "non-substitution rule missing")

        require(unitary.get("schema") == UNITARY_SCHEMA, "unitary specialist schema drift")
        require(unitary.get("control_id") == UNITARY_CONTROL_ID, "unitary specialist control ID drift")
        require(unitary.get("status") == "LIVE_VERIFIED", "unitary specialist state is not LIVE_VERIFIED")
        require(
            (unitary.get("identity_registry") or {}).get("counts") == identity.get("counts"),
        ...
        require(verification.get("state") == "DEPLOYMENT_BUILD_SUCCESS", "production status overclaims or understates deployment")
        require(
            verification.get("current_exact_route_content_verification") == "NOT_RECORDED_FOR_SERVED_SHA",
        ...
        require(specialist_release.get("state") == "LIVE_VERIFIED", "live-verified specialist release missing")
        require(specialist_release.get("control_id") == UNITARY_CONTROL_ID, "specialist release control mismatch")
        require(SHA_RE.fullmatch(str(specialist_release.get("source_publication_sha", ""))) is not None, "specialist source SHA invalid")
        require(SHA_RE.fullmatch(str(specialist_release.get("verification_fix_sha", ""))) is not None, "specialist verification SHA invalid")
        require(isinstance(specialist_release.get("workflow_run_id"), int), "specialist verification run missing")
        require(
            (production.get("specialist_state_routing") or {}).get("unitary_case_and_evidence_state")
        ...
        require(rollback.get("rollback_eligible") is True, "rollback anchor must remain eligible")
        require(rollback.get("state") == "LIVE_VERIFIED", "rollback anchor lost LIVE_VERIFIED state")
        require(rollback.get("source_sha") == HISTORICAL_PR922_SHA, "historical PR #922 SHA changed")
        ...
        for index, release in enumerate(releases, start=1):
            ...
            effective_times.append(parse_time(release.get("effective_at"), f"release {release_id}"))
        ...
        require(len(current_records) == 1, "release ledger must have exactly one current observation")
        require(current_records[0].get("source_sha") == served_sha, "current ledger release does not match served SHA")
        require(HISTORICAL_PR922_SHA in release_by_sha, "rollback anchor absent from ledger")
        require(release_by_sha[HISTORICAL_PR922_SHA].get("rollback_eligible") is True, "rollback release not eligible in ledger")
        ...
        old_production = load(ROOT / "archive" / "ops-snapshots" / "PRODUCTION_STATUS_20260824.json")
        require(
        ...
        for prohibited in ("baseline_main_sha", "baseline_release", "main_at_audit", "exact_live_sha"):
            require(prohibited not in current, f"legacy current-truth key remains at CURRENT_STATE root: {prohibited}")
