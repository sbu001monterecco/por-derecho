...
for path in (DATA, ES, EN, TRANSCRIPT, REGISTER, MANIFEST):
    if not path.is_file():
        fail(f"missing required file: {path.relative_to(ROOT)}")

data = json.loads(DATA.read_text(encoding="utf-8"))
manifest = json.loads(MANIFEST.read_text(encoding="utf-8"))
texts = {path: path.read_text(encoding="utf-8") for path in (ES, EN, TRANSCRIPT, REGISTER)}
...
matches = [row for row in rows if row.get("Master_ID") == "TF-CIV-006"]
if len(matches) != 1:
    fail(f"expected one TF-CIV-006 row, found {len(matches)}")
if len(matches[0]) != len(rows[0]):
    fail("TF-CIV-006 CSV column count is malformed")

for marker, path in (("ME-086", ROOT / "archive/MISSING_EVIDENCE_REGISTER.md"), ("CR-088", ROOT / "archive/CORRECTION_REGISTER.md")):
