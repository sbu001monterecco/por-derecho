...
def project_page(text: str, counts: dict, stamp: str, lang: str) -> str:
    ...
    if lang == 'en':
        gap = '<div class="id-note" id="gitlab-public-registry-gap"><strong>Cross-host registry rule · 23 September 2026.</strong> This page reports the source-controlled GitHub registry currently checked into this branch. GitLab and GitHub have held different recovery/canonical roles and denominators over time; re-resolve the live controlling repository state before relying on another host’s rendered count. Host divergence is a reconciliation state, not evidence that identities may be silently copied, merged or renumbered.</div>'
    else:
        gap = '<div class="id-note" id="brecha-registro-publico-gitlab"><strong>Regla de registro entre hosts · 23 septiembre 2026.</strong> Esta página informa del registro GitHub controlado por fuente que está incorporado en esta rama. GitLab y GitHub han tenido distintos papeles y denominadores de recuperación/canon a lo largo del tiempo; antes de usar el recuento renderizado de otro host, vuelva a resolver el estado vivo del repositorio controlador. La divergencia entre hosts es un estado de reconciliación, no prueba que permita copiar, fusionar o renumerar identidades silenciosamente.</div>'
    text = once(
        text,
