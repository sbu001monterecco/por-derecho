...
def project_page(text: str, counts: dict, stamp: str, lang: str) -> str:
    ...
    # the canonical count substitutions cannot rewrite the 515-vs-381 boundary.
    text = re.sub(
    ...
    if lang == 'en':
        gap = '<div class="id-note" id="gitlab-public-registry-gap"><strong>Verified public freshness gap · 19 September 2026.</strong> Anonymous GitLab Pages renders a later 7 September registry with <strong>515 aggregate IDs</strong> (256 people, 119 organisations, 12 structures, 68 institutions and 60 proceedings). Those aggregate counts prove a deployment freshness difference only. The identities and exact source bytes comprising the 134-ID delta have not been recovered with GitLab object identity, so this GitHub register remains at its source-controlled 381 IDs. <a href="https://github.com/sbu001monterecco/por-derecho/issues/1535">Reconciliation control #1535 →</a></div>'
    else:
        gap = '<div class="id-note" id="brecha-registro-publico-gitlab"><strong>Brecha pública de actualización verificada · 19 septiembre 2026.</strong> GitLab Pages anónimo muestra un registro posterior, fechado 7 de septiembre, con <strong>515 IDs agregados</strong> (256 personas, 119 organizaciones, 12 estructuras, 68 instituciones y 60 procedimientos). Esos totales acreditan sólo una diferencia de frescura del despliegue. No se han recuperado con identidad de objeto GitLab las identidades ni los bytes exactos que componen la diferencia de 134 IDs, por lo que este registro GitHub permanece en sus 381 IDs controlados por fuente. <a href="https://github.com/sbu001monterecco/por-derecho/issues/1535">Control de reconciliación #1535 →</a></div>'
    text = once(
        text,
