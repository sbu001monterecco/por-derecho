...
* downstream public routes, data and runtime scripts that can reintroduce a
  superseded date, remedy, bidder or order-function claim.
...
PUBLIC_DERIVATIVE_HASHES = {
    "FORENSIC_EVIDENCE_INDEX_CONCURSO_36_2012_21AUG2026.csv":
        "3de4cbb85a79356c812b1d0a5105a3b6670cb1e554dcb6d45d96ca53cd6f0916",
    "FORENSIC_SCAN_CRITICAL_AUTOS_CONCURSO_36_2012_21AUG2026.md":
        "4e1a6ff3652d029846343690a82fb496864fcc5023c7f41dd66d59c0f6e63eda",
    ...
    "GAP_CLOSURE_REGISTER_CONCURSO_36_2012_21AUG2026.csv":
        "7799b5876273c1b6cc976262d3cd360b9c2111c123aa6bd5a6b50ec3d14da30e",
    "GAP_CLOSURE_REGISTER_CONCURSO_36_2012_21AUG2026.xlsx":
        "c33a4a9e7e42b8755631dc0af80ddd0eb83769f146b95196471fd4a8c7062cf8",
    ...
    "REVERSE_ENGINEERING_REDIGEST_CONCURSO_36_2012_21AUG2026.md":
        "9133fad3ac4cdb74bf240411c2065a00750f3cb133b0827c765bd19f7310e9c4",
    ...
    "artifact_closure_audit.md":
        "a63fab4983d2d755ca42cea56b8bcf2e0c779d28d66fdd1386948ca64f281c74",
    "connected_gap_scan.md":
        "a9c70cbd8cdc94ae40a3bd6a8c5c04b8baebad9edd4ff45611abc4e4fd63bde9",
    "repo_site_reverse_engineering.md":
        "0940e6b0c982cca823d922cd478735086a2d0b3498bcfcecefbe2e686fd1ebe5",
}

PUBLIC_DERIVATIVE_MANIFEST = "PUBLIC_DERIVATIVE_SHA256SUMS.txt"
PUBLIC_DERIVATIVE_MANIFEST_SHA256 = (
    "bc24141af428858042191d160577047e80c2237bbf5950ee6bfd99f03012711f"
)

EXPECTED_ORDER_IDS = {
    "C36-2012-06-06-DECLARATION",
    "C36-2017-12-19-LIQUIDATION",
    "C36-2018-02-08-CREDITOR",
    "C36-2018-04-16-PLAN",
...
PAGE_MARKERS = {
    ES_PAGE: (
        ...
        "LAJ 14/02",
        "20 días",
        "«15/02» es etiqueta de copia/notificación",
        "04/06/2018",
    ...
    EN_PAGE: (
        ...
        "court clerk 14 Feb",
        "Twenty-day appeal",
        "“15 Feb” is a copy/notification label",
        "4 Jun 2018",
...
def check_json(failures: list[str]) -> None:
    ...
    if data.get("generated_on") != "2026-08-21":
        failures.append(f"{rel(DATA)} generated_on must be 2026-08-21")
    ...
    creditor = order("C36-2018-02-08-CREDITOR")
    if creditor:
        if creditor.get("date") != "2018-02-08":
            failures.append("creditor-substitution order body date must be 2018-02-08")
        if creditor.get("signature_dates") != ["2018-02-09", "2018-02-14"]:
            failures.append("creditor-substitution signatures must be 9 and 14 February 2018")
        if creditor.get("pages") != 3:
            failures.append("creditor-substitution primary copy must have three pages")
        require_terms(
            "creditor-substitution order",
            joined(creditor, "operative_effect_es", "operative_effect_en"),
            ("15 de febrero es", "15 February is", "not the order date", "no fecha del auto"),
            failures,
        )
        require_terms(
...
def check_targeted_stale_claims(failures: list[str]) -> None:
    ...
    date_15 = re.compile(
        r"(?:auto|order).{0,90}(?:15(?:\s+de)?\s+febrero(?:\s+de)?\s+2018|"
        r"15\s+february\s+2018|15/02/2018|2018-02-15)"
        r"|(?:15(?:\s+de)?\s+febrero(?:\s+de)?\s+2018|15\s+february\s+2018|"
        r"15/02/2018|2018-02-15).{0,90}(?:auto|order)"
        r"|2018-02-15.{0,340}(?:CAM|PH122|litigious|acreedor|creditor|article\s+1535)"
        r"|(?:CAM|PH122|litigious|acreedor|creditor|article\s+1535).{0,340}2018-02-15",
        re.IGNORECASE | re.DOTALL,
    )
    date_15_allowed = re.compile(
        r"(?:15(?:/02)?|15\s+(?:de\s+)?febrero|15\s+february).{0,100}"
        r"(?:etiqueta|label|notificaci[oó]n|notification).{0,120}"
        r"(?:no\s+(?:es\s+)?(?:la\s+)?fecha|not\s+the\s+order\s+date)"
        r"|(?:etiqueta|label|notificaci[oó]n|notification).{0,100}"
        r"(?:15(?:/02)?|15\s+(?:de\s+)?febrero|15\s+february)",
        re.IGNORECASE | re.DOTALL,
    )
    hero_date_15 = re.compile(
        r"<p\b[^>]*class=[\"'][^\"']*\beyebrow\b[^\"']*[\"'][^>]*>"
        r"(?P<body>.*?)</p>",
        re.IGNORECASE | re.DOTALL,
    )
    bare_15_in_2018 = re.compile(
        r"15(?:\s+(?:de\s+)?)?feb(?:rero|ruary)?(?:\s+(?:de\s+)?)?2018",
        re.IGNORECASE,
    )
    body_8_in_2018 = re.compile(
        r"8(?:\s*[–-]\s*14)?(?:\s+(?:de\s+)?)?feb(?:rero|ruary)?"
        r"(?:\s+(?:de\s+)?)?2018",
        re.IGNORECASE,
    )
    ...
    for path in public_text_files():
        ...
        for hero in hero_date_15.finditer(text):
            body = compact(hero.group("body"))
            if bare_15_in_2018.search(body) and not body_8_in_2018.search(body):
                failures.append(
                    "15 February retained as a standalone 2018 timeline date "
                    f"without the controlling 8–14 February layers: {rel(path)}"
                )
                break

        for match in date_15.finditer(text):
            # Keep the allowance local to the occurrence. A correction elsewhere
            # in the same file must not excuse a stale timeline row or sentence.
            context = window(text, match.start(), match.end(), 180)
            if date_15_allowed.search(context):
                continue
            failures.append(
                "15 February presented as the creditor-order date without "
                f"body/signature/notification clarification: {rel(path)}"
            )
            break
