...
def apply():
 assert git('branch','--show-current')=='worker/cr-continuity-gapclosure-20260905','Apply only on the isolated worker'
 assert git('rev-parse','origin/main')==BASE,'Main advanced: reconcile first'
 d=load(CANON)
 if d.get('continuity_closure',{}).get('control_id')==CONTROL:return check()
 changed=[];before={};prod=production();review=rows()
 def put(p,v):
  if p not in before:before[p]=sha((ROOT/p).read_bytes()) if (ROOT/p).exists() else None
  changed.append(save(p,v))
 org=load(ORG);r=next(x for x in org['records'] if x['id']==RID);old=copy.deepcopy(r)
 assert r['name']=='RAUDA' and r.get('identity_resolution')=='CARET_PENDING'
 r.pop('status');r.update(name='RAUDA ALSP S.L.P.',aliases=list(dict.fromkeys(r.get('aliases',[])+['RAUDA','RAUDA ALSP','Rauda'])),identity_resolution='CARET_CONFIRMED',identity_sources=[s['id'] for s in SOURCES],registry_sheet='Barcelona B566926',identity_scope='CORPORATE_ENTITY_ONLY_NOT_EVERY_BRAND_ASSOCIATED_ACT',capacity_boundary='The company is identified by official BORME entries. Its exact 2022 mandate, employer/capacity, information flow, debt ownership, acquirer and liability remain source-dependent. No original client mandate is inferred.',identity_resolution_history=[{'as_of':'2026-09-05','prior_record':old,'superseded_only':'Corporate identity uncertainty; case role remains open','source_refs':[s['id'] for s in SOURCES]}]);put(ORG,org)
 q=load(QUEUE);oldtask=next(x for x in q['exact_identity_queue'] if x['id']==RID);q['exact_identity_queue']=[x for x in q['exact_identity_queue'] if x['id']!=RID]
 q.setdefault('completed_exact_identity_tasks',[]).append({'id':RID,'closed_dimension':'CORPORATE_IDENTITY_ONLY','source_refs':[s['id'] for s in SOURCES],'previous_task':oldtask,'remaining_obligation':'PD-GAP-UCF-013/CR/mandate_handover','control_id':CONTROL});put(QUEUE,q)
 idx=load(INDEX);idx['recovery_identity_control'].update(state='CORPORATE_IDENTITY_CONFIRMED_CASE_ROLE_OPEN',superseding_control=CONTROL,source_refs=[s['id'] for s in SOURCES]);put(INDEX,idx)
 g=load(GAPS)
 for p in prod:next(x for x in g['gaps'] if x['id']==p['gap_id']).setdefault('specialist_obligations',[]).append(copy.deepcopy(p))
 g.setdefault('specialist_continuity_controls',[]).append({'control_id':CONTROL,'source_control':CANON,'obligations':[p['obligation_id'] for p in prod],'merits_closed':False});put(GAPS,g)
 cm=load(COMMS);events_digest=sha(js(cm['events']).encode())
 cm.setdefault('specialist_register_links',[]).append({'control_id':CONTROL,'path':CANON,'object_class':'PRIVATE_PROFESSIONAL_RECOVERY_COMMUNICATION','existing_ids':['PD-CR-COM-20220218','PD-CR-COM-20220307'],'native_custody':'PRIVATE','institutional_event_rows_added':0,'rule':'Federated discovery only: private-professional correspondence is not relabelled as authority notice.'});put(COMMS,cm)
 d['professional_recovery_review']['open_evidence']=prod
 d['professional_recovery_review']['identity_update']={'id':RID,'corporate_identity':'CARET_CONFIRMED','case_role':'OPEN_SOURCE_REQUIRED','source_refs':[s['id'] for s in SOURCES]}
 d['continuity_closure']={'control_id':CONTROL,'review_date':'2026-09-05','scope':'THIS_THREAD_REVIEW_CONTROLS_AND_PUBLICATION_GAPS_NOT_ENTIRE_REPOSITORY','source_base':BASE,'sources':SOURCES,'review_controls':review,'review_control_denominator':{'first_handoff':36,'second_handoff':22,'total':58,'registered':58,'new_primary_events_inferred':0},'production_obligations':prod,'source_custody':'Both handoffs and native evidence remain in the authorised private Library/source system. Re-query by package control; no private locator or original is copied into Git.','correction_rule':'Preserve private operational corrections, attempted-versus-effective communication, chronology/knowledge limits, client/obligor separation, prior work and historical-credit corrections. A held derivative is not deleted continuity or a public fact.','prior_release':{'pull_request':1465,'merge_sha':'dc2a720f6ae6477df6ebddb081c4b9c48c6e729a','pages_run':33971512322,'live_run':33971513405,'verified_at':'2026-09-05T14:22:57Z','state':'LIVE_VERIFIED_FOR_ITS_SCOPE','resources':27},'later_publication_lineage':{'pull_request':1464,'merged_successor':BASE,'meaning':'Later unified release. It was preparation lineage before it subsequently merged; do not repeat the obsolete assertion it remains unmerged.'},'paths':{'es':f'/es/cuatrecasas-sun-park/#{ANCHOR}','en':f'/en/cuatrecasas-sun-park/#{ANCHOR}'},'institutional_events_count':len(cm['events']),'institutional_events_sha256':events_digest,'original_professional_roster_sha256':sha((ROOT/ROSTER).read_bytes()),'proof_completion_claim':False,'universal_caret_claim':False}
 put(CANON,d)
 for lang in ['es','en']:
  path=f'{lang}/cuatrecasas-sun-park/index.html';s=text(path);ptn=r'<p class="pd-cr-identity"[^>]*>.*?</p>'
  assert len(re.findall(ptn,s,re.S))==1
  wording=('La identidad societaria de RAUDA ALSP S.L.P. está resuelta mediante BORME; el mandato y función en este asunto siguen abiertos. La reserva anterior se conserva en el historial.' if lang=='es' else 'RAUDA ALSP S.L.P. corporate identity is resolved through BORME; its mandate and role in this matter remain open. The earlier reservation is retained in the history.')
  replacement=f'<p class="pd-cr-identity" data-caepr-id="{RID}" data-caret-state="CARET_CONFIRMED"><small>{wording} <a href="#{ANCHOR}">{RID} · B566926</a>.</small></p>'
  s=re.sub(ptn,replacement,s,flags=re.S);assert f'<!-- {CONTROL}:PANEL:BEGIN -->' not in s
  s=s.replace('</main>',f'<!-- {CONTROL}:PANEL:BEGIN -->\n'+panel(lang)+f'\n<!-- {CONTROL}:PANEL:END --></main>',1)
  label='Continuidad y prueba pendiente' if lang=='es' else 'Continuity and outstanding proof'
  s=s.replace('</h1>','</h1>'+f'<p data-cr-continuity-jump><a href="#{ANCHOR}">{label}</a></p>',1);put(path,s)
 for lang,slug in [('es','metodologia'),('en','methodology')]:
  p=f'{lang}/{slug}/index.html';assert not (ROOT/p).exists(),'Existing method route requires reconciliation';put(p,method_page(lang))
 h=load(HIST);h.setdefault('current_state','PREPARED_PENDING_MERGE');h.setdefault('owner','Por Derecho / controlled historical transition record');h.setdefault('expected_routes',{lang:[x['resource'] for x in h['transitions'] if x['resource'].startswith(lang+'/') and x['resource'].endswith('index.html') and (ROOT/x['resource']).exists()] for lang in ['es','en']});h['compatibility_note']={'control_id':CONTROL,'meaning':'Additive schema compatibility for a historical preparation snapshot. Original state, production flags, transitions and hashes remain; no historical deployment is certified by these fields.'};put(HIST,h)
 m=load(MANIFEST);m['completion_record']={'pull_request':1465,'merge_sha':'dc2a720f6ae6477df6ebddb081c4b9c48c6e729a','pages_run_id':33971512322,'live_verification_run_id':33971513405,'observed_at':'2026-09-05T14:22:57Z','state':'LIVE_VERIFIED','scope':'Original 24 managed pages and three controls; historical observation, not current whole-repository state.'};m['continuity_successor']={'control_id':CONTROL,'canonical_control':CANON,'current_paths':d['continuity_closure']['paths'],'old_generator_policy':'RETIRED_FOR_WRITES_SUCCESSOR_SCOPED_VERIFICATION'};put(MANIFEST,m)
 a=load(AUDIT);a['historical_preparation_snapshot']=True;a['current_verification_control']={'id':CONTROL,'path':CANON,'rule':'Prepared hashes remain provenance; later authorised corrections use scoped successor verification.'};put(AUDIT,a)
 for path in ['scripts/build_cuatrecasas_rauda_20260905.py','scripts/complete_cuatrecasas_rauda_controls_20260905.py']:
  s=text(path);assert '\ndef main():\n' in s;s=s.replace('\ndef main():\n','\ndef main():\n    from close_cr_continuity_20260905 import legacy_entry\n    return legacy_entry()\n',1);put(path,s)
 s=text(WORKFLOW);s=s.replace("if: github.ref == 'refs/heads/integration/cuatrecasas-rauda-20260905' && github.event_name != 'pull_request'","if: ${{ false }} # Historical preparation retired; generate before reviewed merge.")
 assert "get_attribute('data-caret-state')=='CARET_PENDING'" in s
 s=s.replace("get_attribute('data-caret-state')=='CARET_PENDING'","get_attribute('data-caret-state')=='CARET_CONFIRMED'");put(WORKFLOW,s)
 note=text(NOTE)+f'''\n## Superseding continuity and source repair — {CONTROL}\n\nThe preceding preparation state is historical. PR #1465 merged as dc2a720f6ae6477df6ebddb081c4b9c48c6e729a; Pages 33971512322 and live verification 33971513405 succeeded at 2026-09-05T14:22:57Z. PR #1464 subsequently merged unified successor 090a678ad53cd9216e673b0cfc643cd084e5286c; it is no longer correctly described as unmerged. Preserve later Orion/Puzzle work.\n\nCurrent sources, correction denominator and obligations: `{CANON}` / `continuity_closure`. Central obligations: `{GAPS}`. The 36 first-handoff and 22 second-handoff review keys are reconciled without inventing 58 primary events. Both full handoffs and sensitive source/strategy remain in the authorised private Library/source system. Future substantive work must retrieve those packages by control ID, not reconstruct originals from public summaries.\n\nRAUDA retains {RID}; official B566926 entries identify RAUDA ALSP S.L.P., not its specific case mandate. The pending record and queue task remain in history/completed tasks. Role, service, claim allocation, transfer and causation remain open obligations. Historical source literals are not silently rewritten.\n\nThe user-command ^ is broader than the displayed identity caret: audit registration, provenance, relations, chronology, reciprocal links, publication and continuity. A prose mention or ID is not the complete audit. Never claim all is^ without a finite denominator and zero material exceptions.\n\nApply PD-MTCP-20260904-01: refresh main and latest #1428 comments. Explicit single-writer ownership covers a declared branch and file scope. Workers do not append unrelated changes to that branch without integrator acceptance. An interrupted deployment is reconciled against actual successor bytes, never repaired by publishing an older tree. Already-running threads must explicitly refresh; rules do not retroactively update their memory.\n\nOld CR writers are retired. Their --check/--live use the successor verifier. Do not reset complete identity, communications or page files to the preparation snapshot. Preserve later additions while validating owned blocks, IDs and boundaries. Evidence changes only the layer actually proved.\n\nTechnical closure, registration, publication and proof closure are distinct. Seven production obligations remain OPEN_SOURCE_REQUIRED with custodians and tests. Unrelated broad red jobs are not certified repaired. No new universal hard gate, protection change, external communication or filing.\n''';put(NOTE,note)
 put('CHATGPT_START_HERE.md',text('CHATGPT_START_HERE.md')+f'\n\n## Cuatrecasas–RAUDA current continuity\nBefore reusing either 5-September handoff or the former CR generator, read `{NOTE}` and `{CANON}` / `continuity_closure` ({CONTROL}). Corporate identity is source-resolved; case role and seven production obligations remain distinct. Preserve the 36+22 review crosswalk and private custody. Refresh #1428 before publication; do not replay historical whole-file outputs.\n')
 ledger=load('ops/RELEASE_LEDGER.json');releaseid='PD-CR-20260905-01-PR1465'
 if not any(x.get('release_id')==releaseid for x in ledger['releases']):
  ledger['releases'].append({'release_id':releaseid,'source_sha':'dc2a720f6ae6477df6ebddb081c4b9c48c6e729a','source_tree_sha':'2939f5d2a0ba96f7733775be60b05574ad828367','pull_request':1465,'state':'LIVE_VERIFIED','website_content_changed':True,'current_at_observation':True,'rollback_eligible':False,'effective_at':'2026-09-05T14:22:57Z','observed_at':'2026-09-05T14:25:09Z','pages_run_id':33971512322,'live_verification_run_id':33971513405,'live_verification_job_id':101321052645,'scope':'24 CR managed page additions and three controls; 27 matches. Historical observation before later releases.','limits':'Not current whole-repository deployment, identity completeness, legal proof or deadline preservation.'});put('ops/RELEASE_LEDGER.json',ledger)
 d=load(CANON);c=d['continuity_closure'];c['changed_files']=sorted(set(changed+[CANON]));c['prechange_sha256']=before;c['page_projections']={lang:{'path':f'{lang}/cuatrecasas-sun-park/index.html','panel_sha256':sha(panel(lang).encode())} for lang in ['es','en']};c['new_routes']=['es/metodologia/index.html','en/methodology/index.html'];save(CANON,d)
 print('APPLIED_PATHS',json.dumps(c['changed_files']));check()
...
def check():
 ...
 assert all(k in load(HIST) for k in ['expected_routes','current_state','owner']);assert load(MANIFEST)['completion_record']['pull_request']==1465
 for lang in ['es','en']:
