...
The checked-in state files are contracts and last observations. They are not
allowed to masquerade as immutable current truth. This script resolves current
repository and deployment facts locally or, with --live, through GitHub's API.
"""
...
def github_get(repository: str, path: str, token: str | None) -> dict[str, Any]:
    ...
    headers = {
        ...
        "User-Agent": "Por-Derecho-Operational-Truth/1.0",
...
def github_search(repository: str, query: str, token: str | None) -> dict[str, Any]:
    ...
    with urllib.request.urlopen(request, timeout=30) as response:
        value = json.load(response)
    if not isinstance(value, dict):
        raise ValueError(f"unexpected GitHub search response for {url}")
    return value


def identity_counts() -> dict[str, Any]:
    value = load_json(ROOT / "assets" / "data" / "matter-identity-registry-v1.json")
    return {
...
def professional_counts() -> dict[str, Any]:
    value = load_json(ROOT / "assets" / "data" / "legal-professionals-register-v1.json")
    return {
...
def latest_successful_pages_run(repository: str, sha: str, token: str | None) -> dict[str, Any] | None:
    query = urllib.parse.urlencode({"head_sha": sha, "per_page": 100})
    runs = github_get(repository, f"actions/runs?{query}", token).get("workflow_runs", [])
...
def live_repository_state(repository: str, token: str | None) -> dict[str, Any]:
    ...
    pages = latest_successful_pages_run(repository, sha, token)
    return {
        "mode": "LIVE_GITHUB",
        "branch": "main",
        "sha": sha,
        ...
        "latest_successful_pages_run_for_main": pages,
    }
...
def compare_observation_to_live(
    ...
    if observation_sha == live_sha:
        return {"status": "EXACT", "ahead_by": 0, "behind_by": 0}
...
def build_report(repository: str, live: bool, token: str | None) -> dict[str, Any]:
    contract = load_json(ROOT / "ops" / "CURRENT_STATE.json")
    ...
    repo_state = live_repository_state(repository, token) if live else local_repository_state()

    observation = contract.get("repository_observation") or {}
    inventory_observation = contract.get("repository_inventory_observation") or {}
    freshness = contract.get("freshness_policy") or {}
    drift: dict[str, Any] = {
    ...
    if live:
        ...
        age_hours = (dt.datetime.now(dt.timezone.utc) - observed_at).total_seconds() / 3600
        ...
        drift["production_status_matches_latest_successful_pages"] = bool(
            ...
            and pages.get("workflow_run_id") == (production.get("deployment") or {}).get("workflow_run_id")
        )
        drift["within_freshness_policy"] = bool(
            comparison.get("status") in {"ahead", "identical", "EXACT"}
            and int(comparison.get("ahead_by") or 0)
                <= int(freshness.get("max_main_ahead_commits_before_refresh", 0))
            and age_hours <= float(freshness.get("max_observation_age_hours_before_refresh", 0))
        ...
        try:
            subprocess.check_call(
                ["git", "merge-base", "--is-ancestor", str(observation.get("sha")), "HEAD"],
                cwd=ROOT,
    ...
    return {
        ...
        "corpus": {
            "identity_registry": identity_counts(),
            "legal_professional_register": professional_counts(),
        },
        "controls": {
            "current_state_schema": contract.get("schema"),
...
def main() -> int:
    ...
    print(text, end="")
    if args.fail_on_drift and args.live and not report["drift"].get("within_freshness_policy"):
