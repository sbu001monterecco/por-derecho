...
def main() -> None:
    ...
    require(audit.get("publication_state") == manifest.get("current_state"), "Audit and manifest publication states differ")
    require(queue.get("publication_state") == manifest.get("current_state"), "Queue and manifest publication states differ")
    if manifest.get("current_state") == "LIVE_VERIFIED":
        closeout = manifest.get("publication_closeout")
        require(isinstance(closeout, dict), "LIVE_VERIFIED manifest lacks publication closeout")
        release = closeout.get("evidence_queue_release")
        require(isinstance(release, dict), "LIVE_VERIFIED manifest lacks evidence-queue release")
        require(release.get("pull_request") == 1301, "Unexpected evidence-queue release PR")
        require(release.get("reviewed_tree_sha") == "f2d36ad08967f09adbec1fbc38e290ca74caa203", "Unexpected reviewed release tree")
        require(release.get("merge_sha") == "024fa29aa64500ce46559e3cf2b1a02ba609a0ae", "Unexpected evidence-queue merge SHA")
        require(release.get("pages_run_id") == 33466780778 and release.get("pages_conclusion") == "success", "Pages release evidence mismatch")
        live = closeout.get("live_verification")
        require(isinstance(live, dict), "LIVE_VERIFIED manifest lacks live readback")
        require(live.get("result") == "LIVE_VERIFIED", "Live readback result mismatch")
        require(live.get("html_routes_http_200_exact_bytes") == 16, "Live HTML denominator mismatch")
        require(live.get("public_json_resources_http_200_exact_bytes") == 3, "Live resource denominator mismatch")
        require(live.get("total_http_200_exact_bytes") == 19, "Live total denominator mismatch")
        require(len(live.get("sha256", {})) == 19, "Live hash denominator mismatch")
        require((ROOT / closeout.get("closeout_record", "")).is_file(), "LIVE_VERIFIED closeout record is missing")
        require((ROOT / queue.get("publication_closeout", "")).is_file(), "Queue publication closeout is missing")
    ...
    require(ricardo_queue.get("publication_state") == manifest.get("current_state"), "Ricardo actor and manifest publication states differ")
    require((ROOT / ricardo_queue.get("publication_closeout", "")).is_file(), "Ricardo actor publication closeout is missing")
