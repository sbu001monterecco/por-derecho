...
def main() -> int:
    try:
        ...
        if counts.get("total") != 230 or counts.get("PERSON") != 95 or counts.get("ORGANISATION") != 79:
            fail("canonical registry counts do not reflect professional extension")
        if len(parts) != 230:
            fail("canonical registry part total is not 230")
        ...
        print(" - canonical identities: 230 (95 people; 79 organisations)")
