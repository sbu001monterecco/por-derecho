...
def main() -> int:
    try:
        ...
        if counts.get("total") != 221 or counts.get("PERSON") != 94 or counts.get("ORGANISATION") != 76:
            fail("canonical registry counts do not reflect professional extension")
        if len(parts) != 221:
            fail("canonical registry part total is not 221")
        ...
        print(" - canonical identities: 221 (94 people; 76 organisations)")
