...
def main() -> int:
    try:
        ...
        if counts.get("total") != 214 or counts.get("PERSON") != 94 or counts.get("ORGANISATION") != 74:
            fail("canonical registry counts do not reflect professional extension")
        if len(parts) != 214:
            fail("canonical registry part total is not 214")
        ...
        print(" - canonical identities: 214 (94 people; 74 organisations)")
