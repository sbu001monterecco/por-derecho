...
REGAGE_STATUS_EXPORT_EXPECTED = 306
...
def base_register() -> dict[str, Any]:
    return {
        ...
        "denominator_control": {
            ...
            "arithmetic_check": "Historical control: 75 detailed + 22 metadata-only = 97 reported total. Current 21-Sep status export: 398 distinct REGAGE rows (92 already represented; 306 added).",
        ...
        "id_allocation": {
            ...
            "extension_rule": "Existing formal-event IDs are never renumbered. The 21-Sep-2026 status export reuses 92 existing IDs, preserves 22 already-reserved REGAGE identities, and assigns 284 remaining rows to the unused PD-SP-EVT-0213..0496 band; mailbox 1001+ remains separate.",
...
def load_regage_status_export_events() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    if source.get("raw_sha256") != REGAGE_STATUS_EXPORT_RAW_SHA256:
        raise ValueError("REGAGE status-export raw source hash control drift")
    if len(records) != 398 or counts.get("records") != 398:
        raise ValueError("REGAGE status-export requires 398 source rows")
    state_counts: dict[str, int] = {}
    for row in records:
        state = row.get("status_literal_es")
    ...
    reused = [row for row in records if row.get("representation") == "REUSE_EXISTING_FORMAL_EVENT"]
    additions = [row for row in records if row.get("representation") != "REUSE_EXISTING_FORMAL_EVENT"]
    preassigned = [row for row in additions if row.get("representation") == "ADD_STATUS_EVENT_PREASSIGNED_ID"]
    generic = [row for row in additions if row.get("representation") == "ADD_STATUS_EVENT"]
    if len(reused) != 92 or len(additions) != REGAGE_STATUS_EXPORT_EXPECTED:
        raise ValueError("REGAGE status-export reuse/add denominator drift")
    if len(preassigned) != 22 or len(generic) != 284:
        raise ValueError("REGAGE status-export preassigned/generic denominator drift")
    generic_ids = sorted(row["event_id"] for row in generic)
    if generic_ids[0] != "PD-SP-EVT-0213" or generic_ids[-1] != "PD-SP-EVT-0496":
        raise ValueError("REGAGE status-export generic ID band drift")
    all_add_ids = [row["event_id"] for row in additions]
    if len(all_add_ids) != len(set(all_add_ids)):
        raise ValueError("REGAGE status-export duplicate added event ID")
    all_refs = [row["registration"] for row in records]
    if len(all_refs) != len(set(all_refs)):
        raise ValueError("REGAGE status-export duplicate registration reference")
    ...
    for row in additions:
        ...
        literal_state = {
            "Recibido": "STATUS_EXPORT_LITERAL_RECIBIDO",
            "Enviado": "STATUS_EXPORT_LITERAL_ENVIADO",
            "Rechazado": "STATUS_EXPORT_LITERAL_RECHAZADO",
        }[status]
        evidence_state = {
            "transmission": literal_state,
            "registration": "REGAGE_REFERENCE_PRESENT_IN_STATUS_EXPORT",
            "filing": (
                "REJECTED_STATUS_PRESERVED_NO_ACCEPTED_FILING_INFERRED"
                if status == "Rechazado"
                else "NO_ACCEPTED_FILING_INFERRED_BEYOND_LITERAL_STATUS"
            ),
        ...
        event = {
            ...
            "cohort": "CURATED_SOURCE_PROVED_EVENT",
            "layer": "FORMAL_REGISTRATION",
            "source_key": f"REGAGE_STATUS_EXPORT_20260921:{ref}",
            "record_type": "OFFICIAL_ACT_UNCLASSIFIED",
            "event_date": presented[:10],
            "presented_local": presented,
            ...
            "matter_references": [],
            "source_integrity": {
                "status": "USER_SUPPLIED_REGAGE_STATUS_EXPORT_PUBLIC_SAFE_DERIVATIVE",
            ...
            "proves": [
                f"The supplied REG/RedSARA export records {ref} with literal status '{status}', the stated destination and timestamp."
            ],
            "does_not_prove": [
    ...
    return events, {
        ...
        "status_literal_es": state_counts,
    }
...
def build_checkpoint(register_sha256: str, source_sha256: str, mailbox_index_sha256: str) -> dict[str, Any]:
    return {
        ...
        "source_required_and_normalisation_gates": [
            ...
            "Historical 31-Aug scan found no post-notification E.G. 745 native receipt; the 21-Sep status export now records REGAGE rows but does not by itself prove a native receipt or downstream handling.",
        ],
