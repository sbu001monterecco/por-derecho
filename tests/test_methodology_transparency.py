...
class MethodologyTransparencyTests(unittest.TestCase):
    ...
    def test_stage_removal_fails_closed(self):
        ...
        def altered(path):
            obj=original(path)
            if path.name=="methodology-transparency-v1.json":
                obj=json.loads(json.dumps(obj)); obj["recursive_sequence"]=obj["recursive_sequence"][:-1]
            return obj
    ...
    def test_architecture_history_cannot_hide_draft_status(self):
        ...
        def altered(path):
            obj=original(path)
            if path.name=="methodology-transparency-v1.json":
                obj=json.loads(json.dumps(obj)); obj["architecture_history"][-1]["status"]="CONTROLLING_ACCEPTED"
            return obj
        with mock.patch.object(M,"load",side_effect=altered):
