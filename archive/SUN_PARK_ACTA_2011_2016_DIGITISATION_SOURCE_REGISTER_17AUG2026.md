# SUN PARK — ACTA / MEETING / AC COMMUNICATION DIGITISATION SOURCE REGISTER

**Date:** 17 August 2026  
**Status:** CANONICAL PUBLIC-SAFE SOURCE INDEX; NATIVE CUSTODY REMAINS IN CONNECTED SOURCE SYSTEMS UNLESS SEPARATELY EXPORTED/HASHED  
**Scope:** authority-lineage evidence from 2011 through 2016, including ACTAS, meeting notices, post-meeting communications, litigation material, Community-debt analysis and communications concerning the Administrador Concursal.

> **27-Aug-2026 completion note:** this register remains a historical 2011–2016 discovery record. Current status is controlled by `archive/ACTA_DIGITISATION_PUBLICATION_CLOSEOUT_27AUG2026.md`, the canonical event ledger, `public-index.json` and the v2 source-family reconciliation. Seventeen located control copies now have separate OCR/text-edition and raster-only redacted source-facsimile layers. For 26-Apr-2016, two distinct 77-page binaries (`68ff554b…` and `549dae8c…`) are render/text equivalent, not byte-identical; 24-, 47- and 50-page partial packages remain separate.

## 1. Purpose

This register prevents the ACTA-authority thread from depending on conversational memory. It records the concrete source family located in Gmail/Drive on 17-Aug-2026 and the evidential consequence of those sources without copying privileged/private binaries into the public repository.

This is **digitisation by source indexing + readable text recovery + evidential mapping**, not a claim that every native attachment has been exported into a private vault or hashed. Native-vault custody must be recorded separately under `archive/EVIDENCE_CUSTODY_AND_PRESERVATION_PROTOCOL_16AUG2026.md`.

## 2. High-value source family recovered

### A. 22-Jun-2011 ACTA + 2011 judgment context

**Gmail thread:** `1540ce64bb1cb313` — subject `Re: Informe legal` — 12/13-Apr-2016.  
**Source status:** DIRECT EMAIL + ATTACHMENT METADATA; body fully readable through Gmail connector.  
**Attachments identified:**

- `7. ACTA 22 JUN 2011.pdf` — PDF, 4,549,723 bytes;
- `SENTENCIA ASUNCION y CIA MAR2011.pdf` — PDF, 4,069,760 bytes.

**Material direct content from the email body:** Patricia's contemporaneous 2016 communication to PwC identified the June-2011 ACTA as a central authority/debt source and explicitly contrasted it with the March-2011 judgment. The email states the party position that AAS had assumed the presidency in Feb-2011, that LPB was deprived of voice/vote, and that the June ACTA itself addressed the exploitation agreement and allocation of expenses. These statements remain PARTY ACCOUNT / CONTEMPORANEOUS ANALYTICAL COMMUNICATION until each attachment is independently transcribed and source-controlled.

**Digitisation action required:** full page-by-page transcription and ACTA X-RAY of `7. ACTA 22 JUN 2011.pdf`; extract exact convenor, attendees/proxies, coefficients, exclusions, voting denominator, resolutions, signatures and references to exploitation/debt. Preserve the attached judgment separately and cross-reference only propositions it actually decides or records.

### B. November-2015 ACTA + April-2016 meeting notice / invitation package

**Gmail message:** `155270de97a23680` — subject `Luchy 2 de 4 - documentación del juicio pendiente que afecta el crédito de la comunidad en el concurso` — 6-Jun-2016.  
**Attachments identified:**

- `DOCS-#9481385-v2-Luchy_Playa_Blanca__S_L__-_Complejo_Sun_Park_-_Junta_General_Extraordinaria_19_11_2015.PDF`;
- `DOCS-#9481468-v2-Luchy_Playa_Blanca__S_L__-_Complejo_Sun_Park_-_Junta_General_Extraordinaria_19_11_2015_-_Bandama.PDF`;
- two delivery-copy PDFs of burofaxes to Asunción Aizpurúa dated 18-Nov-2015;
- litigation and impugnation material including the large `Demanda Impugnación junta 2014.pdf` family.

**Gmail message/thread:** `1541a132422e7102` / `154291cb9c1dab86` — subject `URGENTE: informe y análisis de la situación - acta de noviembre 2015 y convocatoria de junta 26 de abril 2016` — 15–20-Apr-2016.  
**Direct readable content:** the 15-Apr communication records receipt at Sun Park of notifications from Pamanil concerning the November-2015 ACTA and the convocation of a meeting for **26-Apr-2016 at Princesa Yaiza**. It states that copies were attached across two emails because of size. The PwC reply on 18-Apr-2016 expressly asks for detailed accounting proof and asks whether the claim impugning the **22-Jun-2011 Community resolution** is available.

**Additional attachment family in 15-Apr message:** `informe-análisis legal - deuda comunidad 2016.pdf/.docx` plus multiple contemporaneous JPG images, likely photographed pages/notifications. These require image-by-image identification before any assumption that every image is an ACTA page.

**Digitisation action required:** recover the full 26-Apr-2016 notice/agenda package, all attachments/images from the 15-Apr sequence, and ACTA/minutes produced after the 26-Apr meeting; link each invitation→attendance→meeting→minute→post-meeting communication as one event chain.

### B2. 21-Apr-2016 AC refusal to authorise an LPB/Luchy PwC engagement

**Gmail forwarded copy:** `1a0294bdf7a986f8` — original quoted subject `Re: CONVOCATORIA SUN PARK` — original quoted reply dated 21-Apr-2016 at 09:47; forwarded to the connected mailbox on 22-Aug-2026.

**Dedicated source record:** `archive/BORJA_PWC_SUN_PARK_JUNTA_21APR2016_PRIMARY_EMAIL_RECORD_22AUG2026.md`.

Borja Rodríguez-Batllori states that Gil did not require AC permission merely to attend the 26-Apr Junta, but asserts that attributed unpaid Community fees prevented voting. He expressly refuses to authorise contracting the copied PwC lawyers on the information then supplied, citing lack of treasury, possible estate credits, no demonstrated need and no proposed-fee information; he asks Carlos Saavedra, Miguel Hernández and Elena Santos not to provide services to Luchy without his express consent. He also asserts the prior status of his own court-approved estate-credit fees.

**Controlled consequence:** this directly joins the pre-meeting debt/voting issue to the proposed PwC engagement and gives the copied PwC professionals contemporaneous notice of the AC's position. It does not prove that later PwC attendance breached the instruction: the signed ACTA records PwC professionals representing named private owners, and PwC billing uses Matkator client terminology for the 26-Apr service. The actual engagement, payer, scope and any later AC consent remain open.

**Attachment control:** the forwarded message's three unique PNGs are inline signature/rendering assets only. The substantive document mentioned in the earlier quoted email is absent from this forwarded copy and remains a retrieval target.

### C. 2016 accounting / source-demand communications

**Gmail thread:** `154291cb9c1dab86`, especially messages `154304f5105cf3a1`, `15432cf387d0942c`, `15432d8b5ed87cb6`.  
**Source status:** DIRECT CONTEMPORANEOUS EMAILS.

The thread records a specific proposed source-production list to the Community/Pamanil side, including:

- ACTAS of all owners' meetings 2008–2015;
- accounting books and IGIC material 2008–2016;
- supplier/service contracts;
- debt certificates;
- invoices to owners and service invoices;
- Community statutes;
- permitted-use / planning certifications.

Jonathan Simó's direct reply states that the requested material could be requested and gives his contemporaneous expectation that much of it might not exist/be held. This is not proof of non-existence. It is evidence that the source-completion problem was already expressly identified in April-2016.

**Significance:** this materially strengthens the historical knowledge/notice chronology: the need to test the Community debt and governance through underlying books, invoices, ACTAS and authority records was being articulated to advisers in 2016, not constructed retrospectively in 2026.

### D. June-2016 Community-debt / AC escalation / planned burofax

**Gmail thread:** `15544ecc55644ae9` — subject `Reunión con el AC` — 12-Jun-2016.  
**Participants include:** Summers Villages/Sun Park and Cristo A. Suárez Pimentel.  
**Classification:** PRIVATE / LEGALLY PRIVILEGED OR POTENTIALLY PRIVILEGED. DO NOT PUBLISH FULL BODY OR QUOTE LEGAL ADVICE WITHOUT PRIVILEGE REVIEW.

**Public-safe evidential consequence only:** the thread proves a contemporaneous June-2016 plan to:

- put the Community-debt/authority dispute formally before the Administrador Concursal;
- arrange a meeting with the AC involving legal/accounting advisers;
- send a written burofax/warning concerning the disputed Community debt and the AC's treatment of it;
- seek underlying verification/audit of Community invoices, contracts, payments and receipts;
- preserve the concern that Community authority and debt assertions were being used beyond what the project believed the ACTAS authorised.

The thread also shows Cristo commenting on the legal framing and expressly discussing the AC's diligence/responsibility and the possibility of an independent expert/audit. Those legal-advice contents remain privileged/restricted; future public use should state only the fact of advice/notice strategy if legally appropriate.

**Important open point:** this thread documents drafting/planning and the desire to send the burofax. It does **not by itself prove the final burofax was actually sent to or received by the AC**. The final signed/sent burofax and delivery proof remain priority evidence targets.

### E. June-2016 transmission to concursal lawyer + supporting court/ACTA corpus

**Gmail messages:** `155270b982a28996`, `155270de97a23680`, `155270eec5a104b1` — 6-Jun-2016.  
**Source status:** DIRECT SENT EMAILS + extensive attachment metadata.

These emails transmit a large package to Juan Tomás Parrilla concerning the litigation affecting the Community credit in the concurso. The attachment family includes:

- Community ACTA/registry materials;
- 2014 impugnation proceedings;
- the 19-Nov-2015 extraordinary-meeting ACTAS;
- 18-Nov-2015 burofaxes to Aizpurúa;
- procedural/court documents;
- a document titled `Abogado concursal de Luchy Playa Blanca - Rosa y AC.pdf`.

**Significance:** confirms the Community authority/debt dispute and supporting documentary corpus were actively transmitted into the concursal-lawyer workstream by June-2016. It does not establish which exact documents the AC personally received or read; that bridge must be proved separately.

### F. 2016 meeting/debt analysis after 26-Apr meeting

**Gmail messages:** `1553c7d8436c9a0d` / `1553f05644a46549` — 10–11-Jun-2016 — subject `Deuda de Comunidad según Junta del 26 de Abril de 2016`.  
**Attachments:**

- `Análisis Gastos de la Cdad Explotación v3.docx`;
- `Propuesta de Deuda Comunidad - Junta Abril 2016 (2).xlsx`.

**Significance:** proves a contemporaneous post-meeting analytical workstream testing the Community debt figures attributed to the 26-Apr-2016 meeting. The attachment contents still require controlled extraction and reconciliation before publishing any numerical conclusion.

### G. AC meeting scheduling evidence

**Gmail messages:** `155770bc9a786fba` and related thread `15571b5b4be8e72b` — 22-Jun-2016.  
**Direct metadata/snippet:** Juan Tomás reports having received a response from Borja concerning proposed meeting dates.  
**Significance:** supports the existence of an active effort to arrange direct AC engagement after the June source/analysis work. Read the full thread before relying on exact dates or who attended.

## 3. Drive corroboration / discovery anchors

Drive searches on 17-Aug-2026 located relevant later analytical and source bundles, including:

- `Informe_Forense_Maestro_Participacion_Indirecta_Comunidad_Sun_Park_Concurso_36_2012_15JUL2026.pdf` — Drive ID `1w-UUZab0qqkNpJGOhszhq7v6f4RrV19o`;
- `02A_Anexo_Analitico_Documental_Comunidad_CAM_AC_16JUN2026.docx` — Drive ID `1jnT4Jiib1tuhur0GNAkpgzwgB4bX5nkx`;
- `Guión - ProcConcursal - íntegro y cronológico.xlsx` — Drive ID `1TR9WDd7BaBSkeFfPqRrqeh96aTpu5rLG`;
- `PREGUNTAS Y RESPONSABILIDADES AC - 31 Julio 2018` — Drive ID `1GugPA78Z0bMengSq_wTJHnA7BgaZsRZ2F0xAzuRkDQw`.

These are useful retrieval maps and later analytical sources, but they do not substitute for the native 2011/2015/2016 ACTAS, notices, emails and delivery records.

## 4. Digitisation standard for every ACTA event

A future completion pass must preserve four linked layers:

`PRE-MEETING NOTICE / INVITATION / AGENDA`
→ `ATTENDANCE / PROXIES / OWNERSHIP-COEFFICIENT SNAPSHOT`
→ `SIGNED ACTA / RESOLUTIONS / VOTE`
→ `POST-MEETING IMPLEMENTATION / EMAILS / BUROFAX / LITIGATION / AC COMMUNICATION`

For every ACTA, create an ACTA X-RAY record containing:

1. exact filename and source-native ID;
2. date/body/type of meeting;
3. convenor and claimed authority;
4. notice method/date;
5. attendees and proxies;
6. unit/coefficient ownership represented;
7. excluded owners/votes and stated reason;
8. voting denominator and arithmetic;
9. each resolution separately;
10. signatories and certification chain;
11. referenced debt/accounting basis;
12. referenced exploitation/statutory agreement;
13. later persons/institutions relying on the ACTA;
14. legal/procedural challenge and outcome, if any;
15. evidence classification and limitations.

## 5. Custody status

**Repository continuity:** now strong once this register is merged.  
**Readable email recovery:** PARTIAL/STRONG for identified Gmail threads.  
**Attachment identification:** STRONG for filenames/metadata listed above.  
**Full attachment transcription:** INCOMPLETE.  
**Native private-vault export:** NOT CLAIMED COMPLETE.  
**SHA-256 manifest:** NOT CLAIMED COMPLETE.  
**Final 2016 AC burofax + proof of delivery:** OPEN / PRIORITY.

Do not describe identified Gmail attachments as `vault-preserved` merely because they remain available in Gmail. Connected-source availability is not independent evidence custody.

## 6. Priority retrieval queue

1. Full binary/text extraction of `7. ACTA 22 JUN 2011.pdf`.
2. Locate the Feb-2011 meeting/ACTA said in the April-2016 email to be the presidency threshold.
3. Locate original notice/agenda for the 22-Jun-2011 meeting and ownership/coefficient schedule.
4. Fully extract both 19-Nov-2015 ACTA variants and determine why two versions exist.
5. Recover the complete 26-Apr-2016 convocation/agenda/notification package from the 15-Apr emails/images, including the substantive document mentioned in Gil's 21-Apr 00:21 email but absent from Gmail forward `1a0294bdf7a986f8`.
6. Locate the native/original 21-Apr-2016 Borja email with complete RFC 822 headers and determine whether any later AC consent or refusal followed fuller PwC scope/fee disclosure.
7. Locate and extract the signed ACTA of 26-Apr-2016 and any subsequent certification/debt schedule.
8. Extract `Análisis Gastos de la Cdad Explotación v3.docx` and `Propuesta de Deuda Comunidad - Junta Abril 2016 (2).xlsx`.
9. Locate the final June-2016 burofax to the AC, exact sender/signatory, content, transmission receipt and delivery proof.
10. Recover direct AC response(s), meeting confirmation, meeting attendance/notes and post-meeting follow-up.
11. Crosswalk all identified ACTAS against later 2018/2019/2022 ACTAS/authority assertions and current office-holder chain.

## 7. Privilege/publication control

The PwC/Cristo/lawyer communications may contain legal advice, litigation strategy and privileged material. The public repository may retain source IDs, dates, participants, subject lines, attachment metadata and narrowly necessary evidential consequences, but must not publish full privileged communications or legal-advice quotations absent deliberate privilege review.

The fact that a legal adviser suggested a theory does not establish the theory. The evidential value here is principally contemporaneous notice, source requests, chronology, documentary transmission and intended verification steps.

## 8. Fresh-thread retrieval rule

Any future ACTA-authority digitisation pass must read:

- `archive/SUN_PARK_ACTA_AUTHORITY_LEGITIMACY_VISUAL_CONTROL_17AUG2026.md`;
- this register;
- `archive/MISSING_EVIDENCE_REGISTER.md` ME-044–050 / especially ME-047;
- `archive/EVIDENCE_CUSTODY_AND_PRESERVATION_PROTOCOL_16AUG2026.md`;
- `archive/CALIFICACION_ALLEGATION_03_UNITARY_COMMUNITY_PRIVATE_ACTORS_AC_CAUSATION_16AUG2026.md`;
- then re-query the Gmail IDs and Drive IDs above rather than relying on summaries.

**Control rule:** source discovery is not full digitisation; full digitisation requires complete readable extraction of the original ACTA/notice/communication and, for custody resilience, a separately preserved native original with integrity metadata.
