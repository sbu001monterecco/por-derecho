# THREAD-DELETION CONTINUITY AUDIT — CALIFICACIÓN / AP STATUS / DAMAGE

**Date:** 16 August 2026  
**Scope:** current appellate status of Sentencia 163/2023; strengthened opening allegations; already-materialised consequences; January-2025 elEconomista instrumentalisation.

## Deletion-safe recovery points

A fresh Project Sun Rock / Por Derecho thread must recover the following without relying on chat memory:

1. The calificación appeal is in **Audiencia Provincial de Las Palmas, Sección Cuarta, RPL 2523/2025**.
2. Gmail `1995278022ce027a` (16-Sep-2025, Javier Sixto) states that the calificación file was located in the Audiencia Provincial and identifies an attachment using `2025_0002523_RPL`.
3. Gmail `199e6fa989419f60` (15-Oct-2025, Javier Sixto) expressly identifies `SECCION CUARTA DE LA AUDIENCIA PROVINCIAL - RECURSO DE APELACIÓN - 2523/2025`.
4. Gmail `19e20833780604d3` (13-May-2026, Javier Sixto) states that deliberation and judgment were fixed for **4-Jun-2026**.
5. Targeted Gmail searches through 16-Aug-2026 located **no appellate judgment or terminating resolution for RPL 2523/2025**. Latest controlled status: awaiting appellate decision/notification. This status is superseded immediately by a later native/certified appellate ruling.
6. Do not merge RPL 2523/2025 with later Sixto correspondence concerning RPL 3304/2025 or 3319/2025.
7. The public opening is intentionally first-person and allegation-based. Strongest Judge formulation: Gil Marer accuses Alberto López Villarrubia of knowingly signing Sentencia 163/2023 with materially false factual/causal findings and alleges that the conduct amounts to judicial prevarication within Article 446 Código Penal. The site must not report a criminal conviction that does not exist.
8. Strong AC formulation: materially false obstruction/non-collaboration/causation narrative, with knowing-falsehood allegation only where contrary facts arose within the AC's own administration or are demonstrably shown to have been communicated to him.
9. Strong Fiscal formulation: two-page Ricardo de Mosteyrín opinion, facial `administrador concursal` contradiction, important later judicial rejections/narrowing, and DI 248 institutional circularity; keep Ricardo separate from Elena Herrera, who signed the DI 248 archive decree.
10. `Pending appeal` does not mean `no consequences`: public page now identifies procedural/defence burden, reputational/institutional stigma and portability/use of the adverse first-instance judgment outside the insolvency proceeding.
11. January-2025 elEconomista: Gil alleges LPAM instrumentalised Sentencia 163/2023 through the CAM/Acosta Matos response channel to stop publication. Preserve CR-021: physical transmitting account/person remains unresolved; do not state LPAM personally pressed send without native proof.
12. A targeted Gmail search for an external registry receipt proving that the June-2026 Judge criminal-complaint package was actually filed located only signed/final self-sent filing packages, not an external filing receipt. Do not state the criminal complaint was formally filed unless receipt/registry proof is later recovered.

## Canonical files

- `archive/CALIFICACION_DI248_CONTRADICTION_LEDGER_16AUG2026.md`
- `archive/CALIFICACION_APPEAL_STATUS_DAMAGE_OPENING_ALLEGATIONS_16AUG2026.md`
- `archive/CALIFICACION_CONCURSO36_PARALLEL_LIVES_PUBLICATION_CONTROL_16AUG2026.md`
- `archive/ELECONOMISTA_ROMERA_MEDIA_TRACEABILITY_15AUG2026.md`
- `archive/CORRECTION_REGISTER.md` (CR-020/CR-021)
- `assets/calificacion-opening-accountability-20260816.js`

## Custody gap

The mailbox facts above are recoverable through connected Gmail and recorded by message ID, but this thread did not create a new native RFC822/hash custody package for the three appellate-status emails. Treat that as a custody enhancement opportunity, not as a deletion-continuity failure.

## Conclusion

**Deletion-safe for the new material in this thread**, subject to the separate custody gap above and the continuing need to update the appellate status immediately if a later AP ruling is received.
