...
3. `archive/OUTBOUND_MEDIA_CORE_PACKAGE_MANDATORY_RULE_23AUG2026.md` whenever any journalist, editor, newsroom, media organisation, media-routing address or journalistic contact is in scope;
...
- **Diego Sánchez de la Cruz** is included neutrally in `archive/JOURNALIST_MEDIA_REGISTER.md`; outreach history remains controlled separately.
- **Ruth Ugalde outreach:** the initial 23-August media package omitted the mandatory maps and webinar; a separately authorised same-thread corrective resend supplied both Spanish PNGs, the direct webinar link, the Por Derecho dossier routes and the evidential limitations. Status remains `SENT / AWAITING RESPONSE`; no substantive response is inferred. See `archive/RUTH_UGALDE_EL_CONFIDENCIAL_OUTREACH_RECORD_23AUG2026.md`.
...
Current high-level open recommendations include:

- neutral bilingual media briefing routes;
- separate Tier-A media packages rather than mixed mass CC distributions;
- a private follow-up intelligence queue that never triggers automatic sending;
- a formal-channel matrix distinguishing email notice, receipt, routing and operative filing;
- current-state sitemap/discoverability/runtime re-audit; and
- consolidation of duplicate legacy live verifiers without removing the current canonical check.
...
5. The current collaboration route is a useful journalist-facing trust surface because it offers journalistic verification, public-sources-first review, no inherited conclusions and correction/right-of-reply routes.
6. One exact authorization permits at most one exact transmission. A resend, correction or follow-up is a new package requiring new authorization.
7. Never send an external test message, even to an invalid address.
8. Package verification must test mandatory requirements, not merely confirm that an incorrectly declared manifest matches the draft.
