# CAM / 7 JUNE 2018 — DIRECT-INSTRUCTION, CREDITOR-IN-POSSESSION, DE FACTO ADMINISTRATION, INSOLVENCY-ADMINISTRATOR APPROVAL AND JUDICIAL-OMISSION CRIMINAL LEAD

**Date:** 23 August 2026
**Status:** `CANONICAL CRIMINAL-ALLEGATION / EVIDENCE-STATUS / PUBLICATION-CONTROL RECORD`
**Proceeding:** Concurso ordinario 36/2012 · Luchy Playa Blanca, S.L.U. (“LPB”) · Juzgado de lo Mercantil nº 1 de Las Palmas de Gran Canaria
**Related public routes:**

- `/en/cam-creditor-control-shadow-administration-judicial-omission/`
- `/es/control-acreedor-cam-administracion-hecho-omision-judicial/`
- `/en/sun-park-takeover-7-june-2018/`
- `/es/toma-control-sun-park-7-junio-2018/`
- `/en/insolvency-classification-parallel-lives/`
- `/es/calificacion-concurso-36-2012-vidas-paralelas/`
- `/en/acosta-matos-perimeter/`
- `/es/acosta-matos-perimetro/`

## 1. Controlling position

Gil Marer and Aweswell expressly allege that the events culminating on 7 June 2018 were criminal in character and cannot be reduced to a civil title dispute, a mercantile disagreement, a horizontal-property conflict or an irregular episode inside Concurso 36/2012.

Their controlling allegations are:

1. **Direct instruction and coordinated implementation.** CAM, through or with natural persons in the Acosta Matos / Community-administration perimeter, including José Daniel Acosta Matos (“JDAM”), Francisco Mario Matos Matas (“FMMM”) and the relevant Laura Acosta Matos actor, directly instructed, coordinated, authorised, procured, adopted or knowingly implemented the 7 June 2018 physical entry, lock replacement, security deployment, key demand, guest control and exclusion at Sun Park.
2. **Creditor crossing into possession and management.** CAM crossed from isolated-unit ownership and secured-credit enforcement into de facto possession, management and exploitation of LPB estate assets and, separately, interference with CEXP, Matkator and other non-LPB rights and property. For comparative explanation, Gil describes this as a **creditor or lender in possession**, without asserting that Spanish insolvency law creates that English label as a standalone formal status.
3. **Shadow or de facto administration.** CAM-perimeter private actors exercised or attempted to exercise functions properly belonging to LPB’s corporate or insolvency management, the hotel operator, the Owners’ Community or third-party owners, while formal officeholders or administrators implemented, followed, tolerated or failed to reverse those acts.
4. **Criminal approval or ratification by the insolvency administrator.** Gil alleges that the insolvency administrator did more than passively fail to object: the administrator participated in the 18 May 2018 security-authority chain, knew or should have known the foreseeable use of that structure, authorised at least a narrower access route, and then approved, enabled, ratified, concealed or failed in a legally equivalent way to prevent and reverse the wider material takeover.
5. **Judicial omission / prevarication allegation.** Gil alleges that the competent judge’s treatment of the evidence, requests for protection, actual-control change and later consequences amounted to **omissionary judicial prevarication**. The legal classification must be tested against the precise resolution, refusal, delay or omission, the evidence demonstrably before the judge, the judge’s legal competence and duty, knowledge of injustice, arbitrariness, intent and causal effect.

These are **Gil Marer / Aweswell criminal allegations**, not judicial findings. The corpus contains multiple and mutually reinforcing types of evidence sufficient to formulate, preserve and request investigation of those allegations. It does not yet close every actor-specific instruction, identity, mental-state, ratification and judicial-prevarication element.

## 2. What the evidence corpus contains

The repository, website, Gmail and connected files contain the following evidential categories:

| Evidence type | Located material | What it can prove | What it cannot prove alone |
|---|---|---|---|
| Contemporaneous communications | Emails sent during the morning and afternoon of 7 June 2018 to legal advisers and others | Real-time report of persons present, broken entrance/reception access, demands for keys, security, guest identification and assertions of whole-complex ownership | Ultimate truth of every observation; hidden instructions; criminal intent |
| Videos, photographs and same-day evidence packs | Two videos attached to the first 7 June email; 14-page photographic/evidence pack | Physical condition, presence and conduct to the extent visible; timing and contemporaneity | Complete command chain; off-camera communications |
| Criminal complaints and amendments | 11 April 2018 complaint; 7 June 2018 amendment; attached transcripts, photographs and allegations | What Gil formally alleged before and on the event; identified witnesses, alleged prior conduct and requested investigation | Guilt or judicial acceptance of the alleged facts |
| Judicial investigative acts | Order admitting or directing investigative steps | That the complaint crossed an initial procedural threshold and specified investigative measures | Conviction, merits finding or endorsement of the whole allegation |
| Community records | 18 May 2018 meeting and security decision; voting/attendance records; officeholder chain | Formal governance mechanism, vote-qualified fraction, security authorisation and officeholders | That the resolution lawfully transferred possession or authorised every later act |
| Insolvency-administrator communications | Administrator’s role in requesting/convening the 18 May route; later denial of the main lock takeover and reference to narrower authorised access | Knowledge and participation in an access/security authority chain; existence of a contrary/exculpatory explanation | Criminal approval of the full event without scope, knowledge and response evidence |
| Creditor and insolvency records | PH122/CAM assignment materials; recognition of CAM credit; liquidation filings and objections | CAM creditor capacity and its use inside Concurso 36/2012 | Ownership or possessory authority over the whole hotel |
| Title and asset records | Individual-finca deeds and schedules; 54 CAM / 190 LPB / 18 third-party control point; Matkator and other title evidence | Fragmented ownership and the distinction between CAM-owned, LPB-owned and third-party property | The complete provenance and validity of every one of the 54 CAM fincas without the full ledger |
| Due-diligence and valuation material | Cuatrecasas DD, expert analyses, valuation challenges and source packages | The title, debt, valuation and operating contradictions known or available before the control event | Adoption of those conclusions by every recipient |
| Post-event continuity and benefit evidence | Security/access chronology, 31-finca transaction, investor representations, RICPE/HNT/MYND, later Community control and operation | Possible persistence, downstream use, project benefit and the need for accounting | Automatic proof that every later actor shared the original purpose |
| Contrary and adverse records | Provisional dismissal and appellate confirmation; AC denials; later adjudication and conveyance | Procedural outcome and strongest contrary case | A universal merits finding that every act and later-discovered evidential bridge was lawful |

## 3. Mandatory identity control — Laura Isabel is not silently converted into Laura Patricia

This is a controlling correction.

The located contemporaneous record uses three different formulations:

- the first 7 June 2018 email says **“la abogada de CAM, Laura Matos”**;
- the filed 7 June 2018 complaint amendment identifies **Laura Isabel Acosta Matos**;
- the repository’s established abbreviation **LPAM** refers to **Laura Patricia Acosta Matos**.

Gil now expressly attributes direct-instruction responsibility to **LPAM / Laura Patricia Acosta Matos**. That allegation must be preserved. However, the presently located primary 7 June pleading does not itself establish that Laura Isabel and Laura Patricia are the same person, that one name was an error, or that both participated separately.

### Publication rule

Until the identity bridge is closed, public pages must use one of these formulations:

> **the Laura Acosta Matos actor identified in the contemporaneous 7 June record—named Laura Isabel in the filed amendment—while Gil separately attributes responsibility to LPAM / Laura Patricia Acosta Matos; that identity and participation bridge remains to be proved.**

or, when brevity is required:

> **Laura Acosta Matos actor (identity variant under verification).**

No public page may state as an established fact that Laura Patricia Acosta Matos was physically present on 7 June 2018 or directly issued instructions merely by silently replacing the name in the primary pleading.

Required records include identification, professional-registration data, CAM mandates, telephone numbers, emails, travel/location evidence, witness confirmation and the original signed complaint annexes.

## 4. Dated unitary chronology

### 18 April 2017 — Community-charge legal basis altered

The located judgment materially altered the historical basis relied upon for Community charges. Its precise operative effect must be read against the later debt certificates, vote exclusions and each affected finca; it is not converted here into a universal declaration that every later charge or vote was invalid.

**Status:** `JUDICIAL ACT / FINCA- AND PERIOD-SPECIFIC EFFECT REQUIRES APPLICATION`.

### 22 May 2017 — facial Pink-to-LPB access/key document

A facially signed Pink Canary Services, S.L. → LPB access/key or possession document is located. Its authenticity has not presently been displaced, but its exact keys, areas, corporate capacity and legal effect remain disputed. It cannot be silently expanded into a transfer of CEXP's master-key system, third-party property or CAM's later whole-hotel possession.

**Status:** `LOCATED FACIAL INSTRUMENT / SCOPE, AUTHORITY AND KEY INVENTORY OPEN`.

### 12 June 2017 — governance, debt and vote architecture

The controlled Owners' Community record shows approximately 87.098% present or represented but only approximately 6.545% treated as vote-qualified. Antonio Cogolludo acted as president, a Cogolludo-linked interest supplied the treasurer and FMMM acted as secretary-administrator. Budgets, charges, certificates, claims and banking powers were processed through that reduced electorate.

**Status:** `DOCUMENTED GOVERNANCE NODE / DEBT, ELIGIBILITY AND AUTHORITY EFFECTS DISPUTED`.

### 2016–2018 — isolated-title bridge remains finca-specific

For properties 8,499 and 8,500, the located 2016 instrument was conditional, April 2017 Registry material still showed Montelanza and a later 2018 schedule showed CAM. The condition-satisfaction, paid consideration and onward-transfer bridge remain necessary. The same discipline applies to the provenance of each of the later 54 CAM properties.

**Status:** `PARTIAL TITLE CHAIN / NO RETROACTIVE WHOLE-HOTEL INFERENCE`.

### 20 October 2017 — credit-assignment bridge presently controlled

The existing creditor-control ledger records the transfer of the relevant secured-credit position from Promontoria Holding 122 B.V. to a CAM entity on 20 October 2017. The exact assignee name, consideration, signatories, notice and legal effectiveness must be controlled from the native deed.

**Status:** `DOCUMENTED CHRONOLOGY / PRIMARY DEED COMPLETION REQUIRED`.

### 31 October 2017 — alleged pre-takeover access episode

The April 2018 complaint alleges that JDAM, Antonio Cogolludo, FMMM, private security and others entered or moved through Sun Park, represented themselves as incoming owners/controllers, spoke to guests about departure and interfered with access or locks. The complaint identifies photographs and a witness.

**Status:** `PARTY PLEADING + REFERENCED CONTEMPORANEOUS EVIDENCE / NOT ADJUDICATED`.

### January–March 2018 — alleged preparation, inspection and concealment requests

The April complaint alleges repeated JDAM approaches, whole-hotel ownership assertions, proposed inspections and measurements, pressure or inducements directed at the onsite manager, and statements concerning an expected handover by the insolvency administrator. It also alleges damage or forced access to LPB-specific property.

**Status:** `PARTY PLEADING / TRANSCRIPTS, CALLS, IMAGES AND WITNESS AUTHENTICATION REQUIRED`.

### 15 February 2018 — CAM recognised as credit holder

The existing control ledger records a Commercial Court order recognising CAM’s mortgage-credit position while rejecting the Article 1535 argument in the form advanced.

**Status:** `JUDICIAL ACT / CREDIT IS NOT WHOLE-HOTEL TITLE`.

### 11 April 2018 — pre-event criminal complaint

Gil filed a criminal complaint against CAM, JDAM and possible participants alleging, among other possibilities, usurpation, damage, extortion, fraud, procedural fraud and disloyal administration. The pleading alleged that a secured creditor was acting as if it already owned the hotel and sought investigation and protective measures.

**Status:** `FORMALLY FILED PARTY ALLEGATION / NOTICE CHECKPOINT`.

### 18 May 2018 — Community/security authority node

The controlled Community record shows:

- approximately 86.715% present or represented;
- only an approximately 0.385% Cogolludo-linked interest treated as vote-qualified;
- Antonio Cogolludo as president;
- FMMM as secretary-administrator;
- the Cogolludo treasurer role;
- authorisation of private security expenditure of up to €8,500 per month;
- and an insolvency-administrator communication asking the president to convene the resolution on LPB’s behalf as majority owner.

This produces an authority inversion:

> **LPB’s majority was invoked to convene and legitimise the measure, while LPB was deprived of effective voting power through the disputed debt mechanism.**

**Status:** `DOCUMENTED GOVERNANCE NODE / LEGAL AUTHORITY AND IMPLEMENTATION SCOPE OPEN`.

### 7 June 2018 — physical entry and material-control threshold

The first contemporaneous email says that CAM’s lawyer “Laura Matos” and Owners’ Community personnel arrived to take possession; broke or forced the entrance/reception access; demanded keys to the complex and premises; checked or sought guest identification; and said the complex was theirs. The email said no judgment, Community minutes or other authority had been produced at that moment.

A later same-day email says the CAM side showed the onsite manager a paper asserting ownership of the whole complex, while the Community side showed an 18 May 2018 minute said to authorise the action.

The filed complaint amendment alleges that Antonio Cogolludo and Laura Isabel Acosta Matos, acting personally and purportedly for the Community and CAM, arrived with security, a locksmith and others, forced access, changed locks and controlled entry.

**Status:** `CONTEMPORANEOUS EMAILS + PARTY PLEADING + VIDEO/PHOTOGRAPHIC CORPUS / EXACT COMMAND CHAIN OPEN`.

### 8–26 June 2018 — legal response after control changed

The legal advisers were notified in real time. Complaints, evidence and requests were transmitted. A later suspension order belongs to the post-event procedural chronology.

**Temporal rule:** a later suspension, adjudication or conveyance does not establish legal authority for the 7 June acts at the earlier date.

### 2018–2021 — alleged persistence and estate/non-estate effects

The corpus contains further access, security, works, exclusion and inspection disputes, including later allegations that security prevented access or inspection. It also contains the separate 31-finca transaction, valuation and sale issues.

**Status:** `CONTINUITY PARTIAL / ACTOR, ASSET, PERIOD AND INCOME LEDGER REQUIRED`.

### 2020–2021 — whole-project representations before the later adjudication

The RICPE Folleto and webinar presented a materially integrated CAM Sun Park project. The July 2021 RICPE-side record later identified the 54 CAM / 190 LPB / 18 third-party split and incomplete acquisition/due-diligence position.

**Status:** `DOCUMENTED REPRESENTATION / KNOWLEDGE AND CORRECTION TEST`.

### 2022 — later Community control, adjudication and project implementation

Later adjudication/conveyance and the February 2022 Community record constitute a legally distinct stage. The Community minutes show direct Acosta Matos voting, JDAM presidency, FMMM's continuing secretary-administrator/banking role and approval of the CAM project and works.

**Status:** `LATER JUDICIAL/NOTARIAL/GOVERNANCE STAGE / NO RETROACTIVE CURE`.

## 4A. Capacity-convergence control

CAM's capacities must be dated rather than projected backwards:

| Capacity | Controlled position | Non-permitted shortcut |
|---|---|---|
| Isolated-unit owner | CAM appears as owner of some individual properties before the later whole-platform adjudication; the 2021 control point distinguishes 54 CAM / 190 LPB / 18 third-party properties | Individual title did not itself transfer another owner's unit, CEXP rights, the hotel business, hotel income or whole-platform possession |
| Community participant or beneficiary | Unit ownership carried corresponding Community participation; the 2018 formal route operated through Cogolludo officeholders, FMMM and security, while direct Acosta Matos governance is documented in February 2022 | Later direct governance is continuity and benefit evidence, not automatic proof that the same natural persons issued the 7 June 2018 operative order |
| Secured creditor | The current ledger places the PH122→CAM assignment on 20 October 2017 and the 15 February 2018 judicial recognition before the control event, subject to completion from the native deed | Credit and enforcement rights did not themselves convey ownership, interim administration or possession of LPB and third-party properties |
| Alleged material controller | 7 June contemporaneous reports, the Community/security chain, later access/use evidence and downstream project benefit support a material-control investigation | Material control is not the same as adjudicated legal possession, whole-hotel ownership or proved criminal instruction |

The unitary hypothesis is therefore temporal:

> **Across the 2017–2022 sequence, CAM's isolated-title, Community and secured-credit positions converged. By the 2018 control events, there is a serious actor-specific question whether creditor and title positions crossed into material possession and management; by 2022, direct CAM Community governance and project control were documented.**

## 5. Direct-instruction allegation — CAM / JDAM / FMMM / Laura actor

### 5.1 What is presently documented

- CAM had a recognised secured-credit position before 7 June 2018.
- JDAM and FMMM are placed by the filed April complaint in earlier access, ownership-assertion and control episodes.
- The 7 June contemporaneous email places a CAM lawyer identified as “Laura Matos” with Community personnel at the physical event.
- The 7 June complaint amendment identifies Antonio Cogolludo and Laura Isabel Acosta Matos as acting personally and purportedly for the Community and CAM.
- The CAM side reportedly produced a paper representing that CAM owned the whole complex.
- The Community side reportedly relied upon the 18 May security minute.
- The event implemented an outcome materially beneficial to the secured creditor and later successor project.
- Later records show direct Acosta Matos Community and project control.

### 5.2 The strongest presently supportable allegation

> **Gil Marer alleges that the 7 June event was not an unauthorised improvisation by a locksmith or security guard. He alleges that CAM, JDAM, FMMM, the relevant Laura Acosta Matos actor and Community-officeholder/service-company participants formed or used a coordinated instruction and implementation chain to convert CAM’s secured-credit and isolated-title position into practical whole-hotel control.**

The allegation is supported by the convergence of prior approaches, contemporaneous presence, asserted representation, Community authority, creditor benefit, later control and absence so far of a located whole-hotel possession order.

### 5.3 What remains necessary to convert inference into proved direct instruction

- native emails, WhatsApp, calls or meeting records issuing instructions;
- CAM Board or management approval;
- legal mandate and power of attorney for the Laura actor;
- security-company and locksmith contracts;
- payer and invoice;
- key handover receipt;
- instructions identifying areas and owners;
- Antonio Cogolludo/FMMM communications;
- witness evidence identifying who gave each order;
- post-event CAM adoption, ratification or internal reporting;
- precise identification of the Laura actor.

The absence of a recovered written instruction does not disprove oral or tacit instruction. It prevents direct instruction from being published as an adjudicated or conclusively proved fact.

### 5.4 Actor/function matrix

| Actor or perimeter | Located or alleged function | What remains to be proved |
|---|---|---|
| CAM entities and identified natural persons | Isolated title, secured credit, alleged preparation/coordination, project benefit and later direct governance | Exact assignee/entity at each date; native instructions; mandate; payer; receipt of keys; operational decisions; income and benefit |
| JDAM | Recorded-call attribution and prior access/project/ownership sequence; later direct governance and benefit | Technical speaker attribution; bridge from March planning to June order; native messages; no physical presence on 7 June is presently established |
| Relevant Laura Acosta Matos actor | Direct-day conduct is alleged in contemporaneous and filed sources | Laura Isabel / Laura Patricia identity bridge; CAM mandate; exact order; denial and other contrary evidence |
| Antonio Cogolludo | Community president and formal authority relay; alleged direct-day conduct | Knowledge of the mandate's limits; communications with CAM, AC, FMMM and providers |
| Cogolludo treasurer/voting interest | Approximately 0.385% vote-qualified interest through which the May resolution was adopted | Knowledge of debt/vote architecture, intended scope and later physical implementation |
| FMMM | Secretary-administrator; debt, voting, records, banking and Community/security continuity | Native provider instructions, physical role, certification/record custody and relationship to the actual control event |
| Pamanil/Pamalexsha | Observable administrative, documentary, maintenance and Community-side functions | Which legal person performed each act, under whose instruction, with what authority, knowledge, reliance and benefit |
| Security company and locksmith personnel | Direct physical implementation of guards, access and lock measures | Identity, contract, payer, orders, zones, key recipient, logs, reports and knowledge of authority limits |
| Insolvency administrator | Requested/advanced the security route and later described a narrower authority while denying the main takeover | Exact authority, scope, supervision, first notice, response, recovery/accounting action, knowledge and intent |
| CAM advisers or lawyers | Possible communication, mandate and transaction roles | Professional representation alone proves nothing; knowing assistance must be shown document by document |
| Montelanza/Molina-linked sellers | Earlier title and retained-property chain | Each later conveyance, representation, instruction and benefit; no automatic participation in the 2018 event |

No family, professional, corporate or commercial association substitutes for proof of instruction, knowledge, contribution and benefit.

## 6. Creditor or lender in possession — functional liability framework

“Lender in possession” is used as a comparative functional description, not as a standalone Spanish statutory status.

The allegation is that a secured creditor crossed from protecting or enforcing collateral into actual possession, direction, management, use or appropriation of assets before formal adjudication.

No instrument has been located in the reviewed corpus authorising CAM to take interim administration or possession under Article 690 of the Civil Procedure Act or an equivalent insolvency authority. That is a corpus finding, not a categorical claim that no such instrument could exist. A mortgage subjects property to enforcement; it does not itself transfer possession or permit appropriation merely by virtue of the security.

The required accounting starts at the first proven material-control date and must address:

- keys and access;
- guest admission and exclusion;
- reservations and deposits;
- rents, revenues and fruits;
- staff and contractor instructions;
- security, maintenance, utilities and insurance;
- works and deterioration;
- operator and tour-operator relationships;
- access provided or denied to valuers, investors, ONA or bidders;
- information asymmetry in later sale/adjudication;
- value generated using LPB and third-party assets;
- expenses charged to the estate, Community or other owners;
- debt set-off or other economic benefit;
- later project, finance and public-support benefit.

Any liability and remedy must be allocated to the correct claimant—LPB/the estate, Aweswell, CEXP, Matkator, Pink, individual owners or another person—and must avoid double recovery.

## 7. Shadow or de facto administration — four separate lanes

### 7.1 De facto administration of LPB as a company

Spanish company law extends administrator responsibility to persons who in reality perform administrator functions without title or under whose instructions the formal administrators act.

The hypothesis requires proof that identified CAM-perimeter persons exercised sufficiently continuous and autonomous LPB management functions or that formal officeholders habitually followed their instructions.

Potential indicators include:

- deciding who could enter LPB property;
- deciding whether the hotel would remain open or close;
- selecting or excluding operators;
- directing security, maintenance and works;
- controlling records, reservations or revenue;
- directing insolvency proposals, disposals or settlement positions;
- using LPB assets in project promotion;
- determining access for bidders, experts or advisers.

Creditor pressure, negotiation or benefit alone is insufficient.

### 7.2 De facto possession or management of insolvency-estate assets

A private actor need not replace LPB’s entire corporate board to incur responsibility for practical control of estate assets. The actor-by-asset test is:

> `OWNER → ESTATE STATUS → FORMAL CUSTODIAN → ACTUAL KEY/ACCESS HOLDER → USE → INCOME → WORKS → TRANSFER → BENEFIT`.

### 7.3 De facto hotel operation or control

The hotel-operation analysis must identify who actually controlled:

- reception and guests;
- booking systems;
- merchant/bank accounts;
- employees and contractors;
- cleaning and maintenance;
- utilities and insurance;
- tourism registrations and licences;
- commercial contracts and inventory;
- income and expenditure.

### 7.4 Interference with assets outside LPB

CAM cannot be “shadow administrator of LPB” in respect of a Matkator or other third-party finca merely because it is located inside Sun Park. The legally correct allegations are unauthorised possession, interference, use, works, exclusion, exploitation or appropriation of third-party property or rights.

CEXP collective/operating rights must also be analysed separately from LPB title.

## 8. Insolvency administrator — criminal approval, enablement, ratification or omission allegation

### 8.1 Positive knowledge/authority bridge already located

The strongest point is not silence alone:

- the insolvency administrator represented or administered the LPB insolvency perimeter;
- the administrator is linked to the request that the 18 May security resolution be convened on LPB’s behalf;
- the administrator later denied ordering or approving the principal lock takeover;
- but referred to a narrower access authorisation concerning maintenance/local areas.

The controlled treatment of the administrator's 31 July 2018 declaration also records selection of a security provider, pursuit of Community ratification and authority for limited entry by Laura Acosta into identified LPB premises for maintenance/supervision. That source must be read in full: it is an affirmative authority-chain node and simultaneously a narrower, potentially exculpatory account.

The narrower authorisation is exculpatory as to a whole-hotel takeover if it was genuinely limited and was exceeded without the administrator’s knowledge. It simultaneously proves that the administrator was part of the access-authority chain.

### 8.2 Gil’s strongest allegation

> **Gil alleges that the insolvency administrator criminally approved or enabled the 7 June operation: first by supplying or facilitating the authority route through the 18 May security decision; then by failing to delimit and supervise it; and, after the alleged excess became known, by failing to recover control, protect estate and non-estate assets, demand an account, reverse the effects or present the material-control change accurately to the court.**

### 8.3 Four-level evidential ladder

1. **Lawful narrow mandate:** security or maintenance access was validly authorised and the physical actors exceeded it without the administrator’s knowledge. No criminal approval follows.
2. **Negligent design or supervision:** the administrator created a foreseeable risk, failed to define limits or failed to supervise. This may support civil, professional or insolvency responsibility depending on duty and causation.
3. **Knowing tolerance or ratification:** the administrator learned of the excess, had legal and practical capacity to intervene, and consciously allowed or retained the result. This may support knowing facilitation or criminal omission if the applicable offence and subjective elements are proved.
4. **Affirmative instruction or common purpose:** the administrator intended the wider transfer of control or coordinated it with private actors. This is the strongest allegation and requires direct or compelling circumstantial proof.

Criminal omission is not inferred from silence, deficient supervision or professional breach alone. It requires the particular protective duty, knowledge, practical capacity, causal equivalence and the mental element of the underlying offence. Criminal negligence is available only where the Criminal Code expressly provides it.

### 8.4 Required administrator records

- original 18 May request and all preceding communications;
- exact proposed resolution text;
- legal advice and purpose;
- security contract and instructions;
- CAM/Community/FMMM communications;
- first notice of forced entry;
- same-day and subsequent responses;
- requests to court, police or Registry;
- key/access inventory;
- accounting demands;
- inspection and preservation measures;
- treatment of Matkator and third-party assets;
- treatment in AC reports, liquidation and calificación;
- any benefit retained after notice;
- all contrary records showing attempted prevention or reversal.

## 9. Judicial omission / “omissionary prevarication” allegation

### 9.1 Attribution

Gil alleges that the competent judge's omission to protect, restore, investigate, account for or causally attribute the change in actual control amounted to **omissionary judicial prevarication**.

That label must be published as Gil’s allegation, not as an established legal conclusion.

### 9.2 Legal classification must be precise

Spanish criminal law separately addresses:

- knowingly issuing an unjust judicial resolution;
- grossly negligent or inexcusably ignorant issuance of a manifestly unjust resolution;
- refusal to judge without legal cause;
- malicious delay intended to achieve an illegitimate purpose;
- and, in appropriate offences, commission by omission where a specific legal/contractual duty or prior risk-creating conduct creates an equivalent duty to act.

There is no safe public shortcut from “the judge did not grant the remedy sought” to criminal prevarication.

Spanish case law has exceptionally examined omission within administrative prevarication where a public official was legally compelled to decide and silence was equivalent to an arbitrary express decision. That doctrine must not be transferred automatically to judicial conduct. Judicial conduct must be classified through the exact resolution, refusal or delay under Criminal Code Articles 446–449.

The presently identified allegation concerns Judge Alberto López Villarrubia. The controlled notice/decision nodes include the 27 April 2017 filing, a reported 13 June 2018 discussion, the 4/17 February 2021 filed allegation/registration, the signed 24 February 2021 order and Sentencia 163/2023. They also include contrary or corrective acts, including the 26 June 2018 suspension, the 24 October 2019 non-validation result and partly estimatory aspects of the 24 February 2021 order. Each act must be tested separately; informal or draft material is not attributed to the judge without receipt proof.

### 9.3 Mandatory judicial proof matrix

For each alleged judicial omission identify:

| Element | Required proof |
|---|---|
| Judge / procedural capacity | Exact judge, court and date |
| Matter properly before court | Pleading, request, evidence and filing receipt |
| Competence and duty | Why this judge had power and a legally relevant duty to decide or protect |
| Resolution, refusal, delay or silence | Exact procedural act or omission |
| Evidence before the judge | What the judge demonstrably knew at that moment—not what was discovered years later |
| Objective injustice or arbitrariness | Why the outcome was not merely legally debatable or erroneous |
| Knowledge / subjective element | Evidence the judge knew the result was unjust or intentionally delayed/refused for an illegitimate purpose |
| Causation | What loss, control change, procedural advantage or benefit followed |
| Contrary explanation | Jurisdiction, evidential insufficiency, procedural limits, appeal availability, workload, legal interpretation or later order |

### 9.4 Strongest presently publishable formulation

> **Gil alleges that the court was repeatedly placed on notice of a creditor/private-actor change in actual control, yet the concursal record and later causation analysis did not provide an equivalent actor-by-actor account of who then controlled access, preservation, operation, works, income and rescue. He alleges that identifiable judicial resolutions, refusals, delay and omissions knowingly preserved or legitimised that private-control result. Whether those acts satisfy judicial prevarication, refusal to judge, malicious delay or any commission-by-omission theory requires the exact evidence-before-judge and subjective proof matrix.**

A judicial error, incomplete reasoning or adverse ruling is not by itself criminal.

## 10. Criminal-law lead map

Each alleged act must be classified under the criminal provision in force when it was committed, subject to the retroactive application of a later law that is more favourable to the accused under Criminal Code Article 2.2. The consolidated 2026 text is a research entry point, not permission to project later wording backwards.

The criminal investigation should test, without prejudging:

- a specific false assertion of disposition power followed by sale, encumbrance or lease and prejudice, where Article 251's elements are met—not mere possession or a vague appearance of ownership;
- administration of another's patrimony beyond conferred powers causing patrimonial damage, where Article 252's authority, excess, loss and intent elements are met—not automatic principal authorship for an actor with no administrative power;
- coercion, damage, occupation/usurpation and arbitrary self-help;
- procedural deception and documentary use;
- frustration of estate recovery or creditor equality;
- insolvency offences;
- knowing facilitation, necessary cooperation, inducement and common purpose;
- corporate attribution through natural persons;
- omission where a legally equivalent duty to prevent or reverse is proved;
- judicial prevarication/refusal/malicious delay under the exact statutory elements;
- arbitrary exercise of an asserted right involving violence, intimidation or force against things, as a matter for investigation under Article 455 where its elements fit;
- the specific treatment of insolvency administrators under Article 435.4 only where an underlying malversation offence, affected estate/creditor interests and the required intent are proved;
- concealment or later ratification;
- subsidy/public-funds offences only if later applications contain the required false or concealed conditions.

Every offence must be mapped:

> `ACTOR → CAPACITY → DUTY/POWER → KNOWLEDGE → ACT/OMISSION → SOURCE → RELIANCE → EFFECT → BENEFIT → SUBJECTIVE ELEMENT → CONTRARY EVIDENCE`.

## 11. Contrary evidence and strongest defences

The repository must preserve:

1. the provisional dismissal of the 2018 criminal proceedings and appellate confirmation;
2. any finding concerning absence of violence, title, authority, intent or offence;
3. the insolvency administrator’s denial of the main lock-takeover instruction and narrower-authorisation explanation;
4. possible valid Community/security authority;
5. CAM’s valid title to individual properties;
6. CAM’s valid secured-credit rights;
7. evidence that onsite actors exceeded instructions;
8. evidence that LPB/Gil retained meaningful access or control;
9. later judicial adjudication and conveyance, prospectively and within their lawful perimeter;
10. evidence that the court lacked a proper request, jurisdiction or sufficient proof at a relevant moment;
11. evidence that the judge ruled, rather than refused or delayed;
12. any prompt AC or judicial action to contain, investigate or reverse the event.

These defences do not erase the need to explain the actual-control transition. They prevent the allegation from becoming a predetermined conclusion.

The legal effect of the 2018 dismissal cannot be derived from the word “provisional” alone. Any renewed route must compare the exact persons and facts, the investigation actually completed, finality and the existence of genuinely new relevant indicia or legally distinct later acts. The dismissal is neither a conviction nor an automatic affirmative declaration that every act was lawful; nor may the website promise that it has no preclusive effect.

## 12. Immediate production requests

### CAM / JDAM / Laura actor / FMMM / Community perimeter

- all 2017–2018 access, security, hotel-inspection and possession communications;
- mandates, powers and CAM Board records;
- telephone and messaging logs;
- 18 May resolution drafting and implementation records;
- security/locksmith contract, invoice, payer and instructions;
- key/cylinder inventory and recipients;
- the paper reportedly shown as whole-complex ownership authority;
- post-event internal reports;
- title list used on 7 June;
- communications with AC and court;
- later use, income, works and project records.

### Insolvency administrator

- full authority and knowledge file described in section 8.4;
- all protective, restorative and accounting measures;
- all explanations for non-action;
- complete calificación and liquidation causation treatment.

### Court / CGPJ / prosecution

- complete filing and resolution chronology;
- original evidence and annexes before each decision;
- hearing recordings and internal procedural metadata where obtainable;
- exact treatment of protective requests;
- timing and reason for each delay, refusal or resolution;
- later use of the control position in sale, adjudication and calificación.

### Security company / locksmith / onsite witnesses

- identity, contracts, orders, logs, photographs, body/CCTV footage, reports, invoices and witness statements.

## 13. Publication rules

1. State the direct-instruction, criminal-approval and judicial-prevarication positions forcefully as **Gil Marer / Aweswell allegations**.
2. Do not state criminal guilt, conspiracy or prevarication as judicially established.
3. Never silently merge Laura Isabel and Laura Patricia.
4. Preserve the 2018 dismissal and strongest contrary evidence with equivalent prominence.
5. Distinguish CAM’s unit ownership, Community capacity, secured-credit status, actual possession, project role and later adjudicated title.
6. Distinguish LPB estate assets from CEXP rights, Matkator and other third-party property.
7. Treat “lender in possession” as a functional comparison and explain the Spanish-law acts and remedies separately.
8. Use `evidence before actor`: no later-discovered evidence is attributed retrospectively to the AC or judge without proof of receipt or knowledge.
9. Publish a right of reply and integrate primary corrections with equivalent prominence.
10. A filing, complaint, investigation or receipt proves its procedural existence, not its merits.

## 14. Confirmation of evidential sufficiency for making the allegations

The present corpus contains enough **types and layers of evidence** to support Gil Marer making and requesting investigation of the three principal allegations:

- direct CAM/private-actor instruction or coordination of the 7 June physical event;
- criminal approval, enablement, ratification or legally equivalent omission by the insolvency administrator;
- knowing judicial omission/prevarication, refusal or malicious delay.

That sufficiency arises from contemporaneous reporting, video/photographic evidence, pre-event and same-day complaints, alleged prior conduct, formal governance and access nodes, creditor/title records, later control and benefit, and institutional decisions or omissions.

The evidence is **not yet sufficient to publish those allegations as proved criminal facts**. The remaining gaps are not hidden weaknesses; they are the exact instruction, identity, scope, mental-state, ratification, evidence-before-judge and causation records that competent authorities and record holders are asked to produce.

In particular, this confirmation does **not** confirm as fact that JDAM, LPAM/Laura Patricia or FMMM gave the operative 7 June order; that the insolvency administrator criminally approved it; or that Judge Alberto López Villarrubia committed prevarication. It confirms the existence of a concrete, multi-source basis to allege and investigate those propositions under the actor-specific matrices above.

## 15. Right of reply

CAM, JDAM, Laura Isabel Acosta Matos, Laura Patricia Acosta Matos, FMMM, Antonio and Shaila Cogolludo, Pamanil/Pamalexsha, the security and locksmith providers, the insolvency administrator, the competent judge/court and every separately named actor retain a full right of reply, identification correction, exculpatory production and source challenge.

A substantive response or primary correction must be attributed, preserved and integrated with equivalent prominence. Silence is not admission.
