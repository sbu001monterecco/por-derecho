# SUN PARK — FLAGSHIP / TEST CASE CONTROL

**Canonical proceeding family:** JV 1260/2011 → Rollo de Apelación 793/2012 → AP Las Palmas Sentencia 89/2014  
**Date:** 17 August 2026; updated 23 August 2026  
**Status:** CANONICAL CASE-SPOTLIGHT CONTROL / PUBLICATION-SAFE WITH QUALIFICATIONS  
**Purpose:** Give the pre-concurso eviction/operator proceeding its own flagship case identity and preserve the exact boundary between documented judicial facts, the plaintiffs' procedural initiative, Project allegations, and the separate criminal/forensic questions now under investigation.

## 1. Why this proceeding is a flagship / test case

This proceeding is not important merely because CEXP/Monterecco received an adverse possessory result. It is a compact test case for the Por Derecho methodology because the same file forces a reviewer to distinguish, at minimum:

1. unit-by-unit registered title;
2. participation or non-participation in the common hotel exploitation;
3. possession and consent;
4. the legal identity of the Comunidad de Explotación del Complejo Sun Park (CEXP);
5. the separate legal identity of Monterecco Sun Park, S.L.;
6. the February-2012 cession/transfer and what it did or did not transfer;
7. the plaintiffs' decision to expand the existing action against Monterecco;
8. what the first-instance court actually decided;
9. what the Audiencia Provincial actually decided;
10. what later actors may have treated those judgments as proving;
11. whether any later use over-read the judgment beyond its actual ratio/findings;
12. whether any alleged procedural fraud, maquinación fraudulenta, estafa procesal, documentary falsity or abuse of proceedings is supported element-by-element rather than rhetorically.

The case is therefore a **flagship reconstruction case**, not a predetermined criminal finding.

## 2. Verified procedural identity

### First instance

- **Court:** Juzgado de Primera Instancia nº 4 de Arrecife.
- **Proceeding:** Juicio verbal (desahucio por precario), nº 1260/2011.
- **NIG:** retained in private source control and withheld from public derivatives as an unnecessary case identifier.
- **Originating claim family:** a primary PDF is controlled in Google Drive under titles including `Juicio Desahucio por Precario 1260-2011 23DIC2011.pdf` and duplicate/copy variants.
- **23-Feb-2012 pleading:** the existing action was expanded against **Monterecco Sun Park, S.L.** after the claimants said they had learned of a change in exploitation.

### Appeal

- **Rollo:** Recurso de apelación 793/2012.
- **Resolution:** Sentencia 89/2014.
- **Date:** 13 January 2014.
- **Origin:** JPI nº 4 de Arrecife, JV 1260/2011.
- **Primary PDF family:** controlled in Drive under titles including `Sentencia Proc1260-2011 13ENE2014.pdf` and `Sentencia Apelación Audiencia Proc1260-2011 13ENE2014.pdf` (encoding variants exist).
- **Source control:** SHA-256 `1bce9ad6111645393ee2f23915b7df05a9a879181f2c19fad83f6b3e8989c1ec`; the original remains private. Public derivative: `evidence/jv-1260-2011/full-text/AP89-2014-JV1260-sentencia-redacted.md`.
- **Operative result:** the Audiencia allowed the claimants' appeal, partially reversed the first-instance judgment, ordered the eviction of both CEXP and Monterecco from the claimants' eighteen units, imposed first-instance costs on Monterecco and made no special appellate costs order.
- **Adverse ratio:** the Audiencia found that Monterecco possessed the apartments and keys, held the extension against it justified, and applied veil lifting because of the close relationship between CEXP, Monterecco and LPB in this dispute.
- **Scope boundary:** the judgment did not decide title or authority over the rest of the mixed-ownership complex, later Community governance, the 2018 events, Concurso 36/2012 or criminal responsibility. Finality and actual enforcement remain separate source questions.

## 3. Plaintiffs' procedural equation / extension — controlling hypothesis

The project must not describe CEXP as simply having "converted into" Monterecco Sun Park.

The central procedural fact to reconstruct is that **the plaintiffs initiated the expansion/equation** by asking the court to extend the existing proceedings against Monterecco after the February-2012 change in exploitation.

The appellate result is not open on that dispute: AP 89/2014 accepted Monterecco's possession, treated the extension as justified and lifted the veil. What remains open is the exact first-instance procedural mechanism, finality/implementation and whether any later actor reused that ratio beyond the eighteen-unit perimeter.

Canonical investigative chain:

`claim against CEXP / then operator`

→ `February-2012 cession/transfer/change in exploitation`

→ `plaintiffs learn of change`

→ `23-Feb-2012 plaintiff pleading expands action against Monterecco Sun Park, S.L.`

→ `court allows/handles the expanded proceeding`

→ `first-instance judgment`

→ `appeal`

→ `AP Las Palmas 89/2014`

The words **conversion**, **succession**, **substitution**, **joinder**, **extension**, **continuation**, **new possessor** and **new operator** are not interchangeable. The exact procedural order/pleading must decide which legal mechanism occurred.

## 4. Key appellate source findings — use with precision

The appellate source is important because the complete page-faithful review records three points that must remain separate:

1. **adjudicated holding:** eviction of CEXP and Monterecco from the claimants' eighteen apartments; first-instance costs against Monterecco; no appellate costs order;
2. a claimant-side perimeter tied to **18 units**; and
3. the judgment's wording that the Comunidad de Explotación **“was constituted 88.96% by the apartments” belonging to LPB**.

These figures are not self-proving conclusions about every legal issue. In particular:

- that 88.96% apartment-composition statement does **not** itself establish authority over the 18 claimant-side units;
- it does **not** establish a valid Comunidad de Propietarios voting majority at every later meeting;
- it does **not** determine the validity of the disputed 2011 governance transition;
- it does **not** automatically validate or invalidate later debt certificates, vote exclusions or office-holder authority;
- the adverse possessory result for the claimant-side units does **not** itself prove the broader later proposition that the claimant/minority perimeter possessed valid authority over the hotel as a whole.

This distinction is one of the reasons the case is a test case.

## 5. What the proceeding can prove — and what it cannot

### Documented / capable of direct proof from the court file

- who sued whom and when;
- which units were included;
- what the plaintiffs pleaded;
- which exhibits were filed;
- what was said about CEXP, possession, hotel exploitation and Monterecco;
- the exact 23-Feb-2012 procedural extension;
- what the defendants admitted, denied or argued;
- the first-instance decision;
- the appeal grounds;
- the appellate findings and dispositive result.

### Not established merely because the proceeding exists

- that the 2011 Community authority chain was valid or invalid;
- that the plaintiffs intentionally deceived the court;
- that any omission was deliberate or legally material;
- that a judicial error was caused by a fraudulent manoeuvre;
- that the proceeding satisfies maquinación fraudulenta or estafa procesal;
- that later actors lawfully used the judgment for propositions wider than those decided;
- that CEXP and Monterecco were the same legal person;
- that later formal title retroactively validated earlier authority.

## 6. Three parallel analyses — mandatory

### A. Civil / possessory / procedural

Test:

- registered title;
- consent to exploitation;
- basis of possession;
- standing;
- effect of the February-2012 cession/transfer;
- procedural expansion against Monterecco;
- scope of the judgment and appeal.

### B. Authority / governance / downstream use

Test:

- whether the plaintiffs or aligned actors simultaneously relied on disputed Community authority;
- whether the 18-unit perimeter was later used to support broader governance or debt propositions;
- whether a narrow possessory judgment was later treated as validating wider authority;
- whether later institutional actors independently re-tested the root facts.

### C. Criminal / investigative / forensic

Test, without presumption:

`act / representation`
→ `objective record at the time`
→ `knowledge`
→ `material omission or manipulation, if any`
→ `judicial understanding`
→ `decision`
→ `economic/procedural consequence`
→ `benefit / harm`
→ `causal link`
→ `alternative innocent explanation`.

Possible legal labels are research outputs only after the elements are proved. Civil invalidity is not criminality; formal judicial validity does not by itself exclude a separate criminal-investigative question.

## 7. Core flagship questions

Every serious review of JV 1260/2011 / AP 89/2014 should answer:

1. Who owned the 18 relevant units at each material date?
2. What 2008 sale/completion or participation obligations affected those units?
3. Which units were inside or outside the CEXP exploitation model and why?
4. What did the plaintiffs know about the hotel-wide exploitation structure before filing?
5. What did the originating 23-Dec-2011 claim say about that structure?
6. What changed in February 2012?
7. What exactly did the plaintiffs ask the court to do to bring Monterecco into the case?
8. Was Monterecco's legal basis of possession/exploitation identical to CEXP's or analytically distinct?
9. What did the first-instance court actually decide about that distinction?
10. What did AP 89/2014 actually decide?
11. Which later actors cited or relied on the judgment?
12. Did later actors use the judgment for propositions it did not adjudicate?
13. Did any actor have contrary primary evidence at the time of later reliance?
14. Would the outcome change if the complete 2008–2011 commercial/title record had been before the court?
15. What evidence would disprove the Project's fraud/abuse hypothesis?

## 8. Keystone source queue

Priority source package for this flagship case:

1. complete 23-Dec-2011 originating claim and all annexes;
2. complete 23-Feb-2012 extension/amendment against Monterecco and the order admitting/handling it;
3. complete defence/contestación and preliminary objections;
4. hearing record/audio/minutes if available;
5. complete first-instance judgment;
6. notice and appeal papers;
7. complete AP 89/2014 judgment;
8. Community certificate identifying the claimant-side units;
9. contemporaneous Registry notes for the 18-unit perimeter;
10. 2008 sale/completion/accession package for those units;
11. 2010 notarial demands and owner responses;
12. tourism complaint/inspection material and expediente 92/11;
13. 3-Jan-2011 detective material if actually relied upon;
14. complete 2011 Community ACTA/attendance/proxy/coefficient/debt package;
15. every later court/AC/Fiscal/administrative document that cites the proceeding or judgment.

## 9. Public presentation rule

The public page must visually place the case on a pedestal as:

**SPOTLIGHT · TEST CASE · FLAGSHIP CASE**

but must also show:

**A FLAGSHIP CASE IS NOT A PRE-JUDGED FRAUD CASE.**

The flagship status comes from its diagnostic value: a small unit-level dispute can be traced through title, exploitation, party pleading, procedural identity, judgment, appellate review and later institutional reuse.

## 10. Book / knowledge-project connection

This case is a natural bridge to:

- **Justice in Pieces / Justicia en fragmentos:** because it demonstrates how different legal proceedings can each decide a narrow slice while later users may treat the fragments as a unitary truth;
- **Law Mower Man:** because it demonstrates why legal-tech reconstruction must separate entity identity, procedural posture, fact provenance and downstream reliance before generating conclusions;
- **Por Derecho knowledge project:** because the case is suitable as a reusable demonstration of the `unit → fact → pleading → decision → later reliance` method.

The books are interpretive/long-form outputs; the repository remains the source-controlled evidential layer.

## 11. Publication-status lock

- Existence/procedural identity of JV 1260/2011: **VERIFIED**.
- 23-Feb-2012 plaintiff expansion against Monterecco: **VERIFIED WITH SOURCE-SCOPE LIMITATION**.
- AP 89/2014 / Rollo 793/2012 / date 13-Jan-2014: **VERIFIED; COMPLETE REDACTED PUBLIC TRANSCRIPT CONTROLLED**.
- Adverse appellate holding — eviction of CEXP and Monterecco from the eighteen claimant units, first-instance costs against Monterecco and no appellate costs order: **ADJUDICATED / PUBLIC WITH THE EXPRESS SCOPE LIMIT ABOVE**.
- 18-unit claimant-side perimeter: **VERIFIED WITH CONTEXT LIMITATION**.
- 88.96% CEXP apartment-composition statement: **VERIFIED WITH CONTEXT LIMITATION**.
- Plaintiffs' procedural equation as the origin of the CEXP→Monterecco litigation bridge: **STRONGLY SUPPORTED / REQUIRES COMPLETE ORDER-AND-PLEADING RECONSTRUCTION FOR EXACT LEGAL CHARACTERISATION**.
- Maquinación fraudulenta / fraude procesal / estafa procesal: **WORKING HYPOTHESES TO TEST; NOT ADJUDICATED FINDINGS**.
- Any actor-specific criminal responsibility: **OPEN / NO COMPETENT FINDING IDENTIFIED IN THIS CONTROL**.

## 12. Downstream consumers

This case control must be consumed by:

- Comunidad instrumentalisation / ACTA authority work;
- 2008 sale-completion map;
- insolvency-origin causation;
- Concurso 36/2012 institutional-accountability analysis;
- Calificación;
- DP 1901/2026 and related prosecutorial reconstruction;
- public-authority clean-room reconstruction;
- book / knowledge-project work;
- website Spotlight architecture.

## 13. Deletion / continuity

Once merged to `main`, a fresh thread must be able to reconstruct without this chat:

- the exact proceeding family;
- why it is a flagship/test case;
- the plaintiffs' role in bringing Monterecco into the existing action;
- the 18-unit vs 88.96% distinction;
- the separation between adverse possessory outcome and wider authority/fraud hypotheses;
- the keystone evidence queue;
- the book/knowledge-project bridge;
- the requirement to preserve exculpatory and contrary evidence.
