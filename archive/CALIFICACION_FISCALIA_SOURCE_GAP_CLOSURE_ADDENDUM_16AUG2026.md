# CALIFICACIÓN / FISCALÍA — PRIMARY-SOURCE GAP-CLOSURE ADDENDUM

**Date:** 16 August 2026  
**Updated:** 17 August 2026  
**Status:** INTERNAL CANONICAL SOURCE-COMPLETION ADDENDUM / CORRECTION GATE  
**Controls:** read with `archive/CALIFICACION_FISCALIA_2012_2019_2026_SOURCE_BUNDLE_RETRIEVAL_GATE_16AUG2026.md`, `archive/CALIFICACION_JAN2026_THREE_SOURCE_PROPAGATION_AND_PICKUP_16AUG2026.md`, `archive/SUN_PARK_NONCONVALIDATION_CALIFICACION_OLAF_PINK_CONTINUUM_FIVE_SOURCE_INGEST_17AUG2026.md`, current `archive/CORRECTION_REGISTER.md` and `archive/MISSING_EVIDENCE_REGISTER.md`.

## Purpose

A fresh connector-backed source-completion scan on 16-Aug-2026 recovered primary institutional/judicial material that closes or materially narrows several gaps preserved by the earlier ten-file CF-01→CF-10 ingest. A 17-Aug five-source follow-up further closes the primary 2021 traceability chain and adds a binary-custody correction for the CGPJ Annex 5. This addendum **supersedes only the specific old missing-source classifications identified below**. It does not convert any criminal complaint or theory into a finding of criminal liability.

---

## 1. 27-Jan-2026 Tenerife Diligencias 20/2026 — PRIMARY DECREE RECOVERED

### Source

Native Gmail attachment recovered from a contemporaneous Fiscalía email/forward chain, subject `DIP 20/2026`:

- attachment reviewed: `document - 2026-01-30T111156.445.pdf`;
- SHA-256 of the recovered attachment: `efd4eee1d4c9f4bddf2d5ca1f0c413f18bbb83ad19d2535b554cdb2f66ffb501`;
- document heading: **Fiscalía Provincial de Santa Cruz de Tenerife**;
- procedure: **Diligencias de investigación preprocesal nº 0000020/2026**;
- NIG: `3803870220260000387`;
- date: **27 January 2026**;
- signed by Fiscal Jefe **José Luis Sánchez Jáuregui y Alcaide**.

### What the decree directly establishes

The decree records receipt on 12-Jan-2026 of a complaint by Gil Marer concerning a hotel/RIC/CNMV-related factual perimeter; describes possible falsity or tax-offence characterisations as matters alleged/for preliminary treatment; states that the alleged facts occurred in Las Palmas; records that ATLANTE showed no criminal diligences in Tenerife for those facts; and expressly orders:

- **incoar Diligencias de Investigación 20/2026**; and
- **inhibir/remitirlas a la Fiscalía Provincial de Las Palmas de Gran Canaria**.

### Correction

The prior CF-source-gate classification:

`REPORTED IN PARTY PLEADING / PRIMARY DECREE MISSING`

is now **SUPERSEDED for this institutional act**.

Current classification:

`SIGNED PRIMARY FISCALÍA DECREE RECOVERED / INCOACIÓN + INHIBITION VERIFIED`.

### Non-upgrade rule

The decree does **not** establish the truth of the underlying RIC, falsity, tax or other allegations. It does not establish judicial prevaricación and must not be represented as prosecutorial acceptance of the broader CF-05 criminal theory. Its verified scope is the institutional act actually stated in the decree.

---

## 2. Valencia → Arrecife / Expediente Gubernativo 19/2026 — SIGNED INHIBITION RECOVERED

### Source

Native Gmail attachment recovered from the Jan–Feb-2026 Fiscalía chain:

- filename: `EG 19-26 DECRETO INHIBICIÓN FISCALÍA DE ÁREA DE ARRECIFE DE LANZAROTE.pdf`;
- SHA-256: `9de1a10231631606501c08feb3cf44e9a3358755d6357401ae388d4d56e8fed7`;
- 5-page scanned institutional PDF;
- heading: **Fiscalía Provincial de Valencia — Jefatura**;
- **Expediente Gubernativo nº 19/2026**;
- decree dated **28 January 2026**;
- signed by Fiscal Jefe **María Pilar Tomás Gómez**.

### What the decree directly establishes

The decree states that on **19-Jan-2026** a complaint submitted by Gil Marer was received through SIR. It characterises the reported facts as banking/financial/contractual matters with patrimonial effects inside and outside Concurso Ordinario 36/2012 and connected with the Sun Park/Mind Yaiza location. On competence, it concludes that the alleged facts occurred in Lanzarote and orders:

- **incoar Expediente Gubernativo 19/2026**; and
- **decretar su inhibición a la Fiscalía de Área de Arrecife de Lanzarote**.

The same recovered PDF includes the signed remittance communication sending the complaint and inhibition decree to the Fiscalía de Área de Arrecife de Lanzarote for whatever investigation it may consider appropriate.

### Correction

The earlier position that the Valencia→Arrecife transfer was merely **reported in CF-06 and required a signed inhibition** is now superseded.

Current classification:

`SIGNED PRIMARY FISCALÍA VALENCIA DECREE / EG 19/2026 INCOACIÓN + VALENCIA→ARRECIFE INHIBITION VERIFIED`.

### Non-upgrade rule

This verifies the **territorial/procedural transfer**, not that Arrecife opened Diligencias de Investigación, performed any specified investigative step, accepted the banking-fragmentation theory, or reached any merits conclusion. The complete Arrecife-side EG 19/2026/current-status record remains a separate retrieval target.

---

## 3. Original AC Plan de Liquidación — PRIMARY DOCUMENT LOCATED AND READ

### Source

Google Drive primary PDF located and directly read:

- `7. PLAN DE LIQUIDACION DE LUCHY 9FEB2018.pdf`;
- Drive ID `1Z2n4dGM6HDAsntMmNEgghxId_MkFh6HO`;
- title inside: **PROPUESTA DE PLAN DE LIQUIDACION DE LUCHY PLAYA BLANCA, S.L.U. — Concurso ordinario nº 36/2012**.

Duplicate/copy variants of the same Plan title also exist in Drive and must not automatically be treated as separate evidence.

### What the Plan directly establishes

The Plan itself states that, despite LPB's then inactivity, the AC considered possible the **unitary disposal contemplated by former art. 148.1 LC**, giving principal treatment to the unitary disposal of establishments/exploitations/productive units.

It identifies LPB's 159 apartments, 29 locales and 2 pools/solarium within Sun Park and records the proposed disposal architecture. This is now a **primary AC-authored source**, not merely language reproduced in CF-10.

### Correction

The prior gap `original Plan de Liquidación` is **CLOSED as a location/substantive-reading gap**.

Current classification:

`PRIMARY AC PLAN LOCATED / DIRECTLY READ`.

This does not by itself close custody/authentication questions such as certified court-file provenance or hash-controlled independent preservation.

---

## 4. Signed 24-Oct-2019 non-convalidation Auto — PRIMARY JUDICIAL DECISION LOCATED AND READ

### Source

Google Drive primary court PDF directly read:

- `12. Auto Cancelacion Venta de Locales - 24OCT2019.pdf`;
- Drive ID `1AkNiggZMZAXJ0U7lefM4U8tFeI6tj5l0`;
- Juzgado de lo Mercantil nº 1 de Las Palmas de Gran Canaria;
- Concurso ordinario 36/2012;
- **Auto dated 24 October 2019**.

### What the Auto directly establishes

The Auto records that:

- AC **escrito nº 1760 of 20-Mar-2019** requested convalidation of the **28-Nov-2018 public deed, protocol 2150, Notary Manuel Guzmán Ramos**, concerning unlinking of properties and subsequent sale between LPB and Construcciones Acosta Matos, S.A.;
- a **12-Dec-2018 Diligencia de Ordenación** existed and had been challenged;
- Aweswell reproduced the issue in **escrito nº 2.987 of 13-May-2019**;
- the Plan/approved architecture treated CAM's proposal as an **`oferta de adquisición unitaria de la unidad productiva de la concursada (sus activos inmobiliarios)`**;
- the apartments were proposed for dación and the locales/pools for direct sale for **€400,000** as components of that unitary acquisition architecture;
- the Auto states that the separate transaction had been undertaken without prior court authorisation, with the court informed after consummation, and that it did not comply with the approved Plan architecture;
- the dispositive section orders **dejar sin efecto** the 12-Dec-2018 diligence and **`No haber lugar a convalidar`** the 28-Nov-2018 unlinking/sale deed.

The Auto also quotes/reports the Audiencia Provincial's 23-Jul-2019 treatment of CAM's proposal as acquisition of the property package **as a productive unit**.

### Corrections / gap status

The following older gaps are now closed or narrowed:

- `signed convalidation decision` → **CLOSED: signed 24-Oct-2019 Auto located/read**;
- `20-Mar-2019 AC escrito 1760/19` → **its existence, date, purpose and requested convalidation are independently verified by the signed Auto, but the original AC filing itself remains a superior-source retrieval target**;
- `12-Dec-2018 diligence` → **its existence and procedural role are independently verified by the signed Auto, but the original diligence remains a superior-source retrieval target**;
- `28-Nov-2018 deed` → **its date, protocol number, notary, parties and transaction type are independently verified by the signed Auto, but the native deed/payment package remains open**.

The original **9-Jan-2019 Registry qualification** remains an independent primary-source target unless separately recovered.

### Evidential consequence for CF-10

CF-10 remains a party pleading, but its reproduced unitary-production/convalidation chronology is now materially corroborated by stronger primary sources: the original AC Plan and the signed 24-Oct-2019 Auto. Future treatment should no longer describe the unitary language or non-convalidation outcome as resting only on CF-10's reproduction.

---

## 4A. 17-Aug follow-up — 24-Feb-2021 works/condition Auto and 12-May-2021 re-tender Providencia now controlled in the same source chain

The 30-page traceability annex supplied on 17-Aug-2026 reproduces complete signed copies of two later primary judicial acts that materially extend the 24-Oct-2019 chain.

### 24 February 2021 Auto

The signed Auto records:

- Aweswell's allegation that the relevant locals/pools perimeter had been demolished/reduced to rubble and the request for judicial inspection and expert valuation;
- alleged inability to access the complex because security prevented entry;
- the AC's contrary position that it had not delivered LPB's assets into CAM's possession and that CAM had acquired numerous third-party apartments, giving it an owner's access position;
- CAM's position that its works concerned its own units / Community safety actions;
- the court's observation that photographs showed works in interiors of uncertain ownership, rubble in common areas and **works in common zones that were the object of the isolated sale the court had not convalidated**;
- denial of the requested inspection and pericial in the liquidation context; and
- the court's express observation that such a request could have sense in a possible responsibility action against CAM or the AC.

**Current classification:** `SIGNED PRIMARY COURT ACT / PHYSICAL-CONDITION AND WORKS DISPUTE BEFORE THE COURT / LIABILITY NOT DETERMINED`.

Do not convert the order into a finding that CAM demolished LPB assets, that CAM/AC were liable, or that the court accepted Gil/Aweswell's full factual account.

### 12 May 2021 Providencia

The signed providencia opens bidding among offerors for LPB's listed fincas and expressly includes the same `locales` and `piscinas con solárium` — registry numbers 8.508–8.536, 8.653 and 8.654 — scheduling the hearing for **18 May 2021 at 10:00**. It fixes the third-party bidder's bond at **€735,694.02**.

**Current classification:** `SIGNED PRIMARY COURT ACT / SAME LOCALS-POOLS PERIMETER AGAIN TENDERED AS LPB PROPERTY`.

### Consequence

The controlled traceability chain is now:

`28-Nov-2018 consummated deed → Jan-2019 contemporaneous objection/notice → 24-Oct-2019 refusal to convalidate → 24-Feb-2021 works/condition dispute before the court → 12-May-2021 re-tender of the same relevant perimeter`.

The finite question is no longer whether those dates/documents exist. It is what acts reconciled the contractual, Registry, possession, physical-works and **€400,000** consequences of the non-convalidated transaction before re-tender, and what equivalent disclosure reached bidders.

---

## 4B. 17-Aug contemporaneous 16-Jan-2019 court opposition — NOTICE SOURCE CONTROLLED

The supplied nine-page `20190116 Denuncia y Oposicion Plan Liquidacion Incumplimiento.pdf` is a contemporaneous party pleading to the Mercantile Court. It expressly raised:

- unitary-disposal architecture;
- the 26-Jun-2018 suspension dispute;
- the 28-Nov-2018 locals/pools sale reported by the AC;
- the stated **€400,000** proceeds and subsequent payments;
- the alleged `ob rem` problem;
- possession/control allegations; and
- the chronology by which the sale preceded the 12-Dec-2018 court-office step.

**Current classification:** `CONTEMPORANEOUS PARTY PLEADING / HIGH-VALUE NOTICE-AND-OBJECTION SOURCE / UNDERLYING ALLEGATIONS NOT AUTOMATICALLY VERIFIED`.

It is important for actor-knowledge chronology but does not substitute for the later signed court and Registry records.

---

## 4C. 28-Jul-2026 CGPJ Annex 5 — filed identity verified, supplied backup is not the exact final filed binary

The official `REGAGE26e00069061338` receipt fingerprints the fifth filed PDF, `05_ANEXO_TRAZABILIDAD_NO_CONVALIDACION_LICITACION_2018_2021.pdf`, with SHA-512:

`5132791ed17013e6a8ed8913c2fdcf711e40fefd0ad5cb98daf4d9f0cf374fcaa5954f7d281d1a2d45becf14f2fc2d3a223cd6cf9d6a1c1d698dcc950fef1039`.

The supplied `(1)` backup copy has a different SHA-512, and the contemporaneous Gmail thread records a later `fixed annex` step before filing. Therefore:

- the receipt proves a final Annex 5 was filed by name;
- the supplied `(1)` file is a relevant pre-filing/backup copy;
- it is **not** the exact receipt-fingerprinted final binary;
- the exact final filed Annex 5 remains a priority recovery/diff target.

See `archive/CGPJ_ALZADA_286_ANNEX5_BINARY_CUSTODY_ADDENDUM_17AUG2026.md`.

This source-integrity point does not alter the independently controlled signed judicial records reproduced in the backup annex.

---

## 5. 12-Jul-2016 quantified AC warning — gap narrowed, not fully closed

Current repository source-completion work already records that a primary **12-Jul-2016 quantified LPB notice** has been located. Google Drive search additionally returns a source package explicitly describing the **burofax of 12-Jul-2016 directed to the AC and proof of sending/receipt**.

Controlled status remains:

`PRIMARY NOTICE LOCATED / COMPLETE NATIVE BUROFAX + ANNEX + RECEIPT RECONCILIATION PACKAGE PARTIAL`.

Do not revert to the superseded proposition that no primary 12-Jul-2016 notice has been located. Equally, do not treat the exact €2,874,076.19 causal breakdown as fully authenticated against every underlying annex until the complete native package is reconciled.

---

## 6. Open targets after this scan

Still open / requiring stronger originals or complete files:

1. certified/full JV 1260/2011 file, hearing, judgment/appeal/finality;
2. complete native 12-Jul-2016 burofax/annex/receipt package and item-by-item €2,874,076.19 reconciliation;
3. complete certified DI 248/2018 expediente, original 17-Oct-2018 complaint, annexes, performed diligences and complete archive file;
4. original CAM offer / Plan Annex II as a standalone native source and original valuation(s), notwithstanding the Plan/Auto quotations;
5. native 28-Nov-2018 protocol-2150 deed and payment/bank evidence;
6. original 12-Dec-2018 Diligencia de Ordenación;
7. original 9-Jan-2019 Registro de Tías qualification as a standalone primary source, notwithstanding its reproduction in the 17-Aug annex;
8. original AC escrito 1760/19 of 20-Mar-2019;
9. complete bank/accounting/refund/set-off/reversal trace for the **€400,000** and related AC/mass-credit payments;
10. complete 18-May-2021 tender pack/acta/audiovisual, bids, attendance and bidder disclosures concerning the 2018 deed, non-convalidation, works/condition and €400,000 history;
11. October-2021 / January-2022 acts and complete final 2022 deed/title bridge;
12. exact final filed CGPJ Annex-5 binary matching the official receipt's SHA-512, followed by a controlled diff against the `(1)` backup;
13. complete Arrecife-side record/current status after Valencia EG 19/2026 inhibition;
14. downstream/current-status record after Tenerife Diligencias 20/2026 inhibition to Las Palmas;
15. complete filed receipts/file references/outcomes for the remaining Jan–Feb-2026 complaint family;
16. source-complete Community accounts/maintenance/operator-cost package and relevant 2017–2018 valuation originals.

`not yet recovered` / `not yet source-complete` must never be rewritten as `does not exist`.

---

## 7. Controlled synthesis after gap closure

The source-completion scan strengthens the repository architecture in a precise way:

- the 2018 **unitary-production architecture is now directly supported by the original AC Plan**;
- the 2018 separate unlinking/sale and later attempted convalidation are no longer dependent only on Aweswell's 2019 pleading: the **signed 24-Oct-2019 Auto directly records and refuses convalidation**;
- the 17-Aug follow-up extends the primary chain through the **signed 24-Feb-2021 works/condition order and signed 12-May-2021 re-tender** of the same relevant perimeter;
- the supplied 16-Jan-2019 pleading adds a high-value contemporaneous **court-facing notice** source but remains a party pleading;
- the exact final CGPJ Annex-5 binary remains open despite verified filing identity, because the recovered backup hash differs from the official receipt fingerprint;
- the Valencia→Arrecife step in EG 19/2026 is now a **verified signed institutional transfer**, not merely a later pleading report;
- the 27-Jan-2026 Tenerife Diligencias 20/2026 act is now a **verified signed institutional decree**, not merely a CF-05 report.

These upgrades strengthen chronology, institutional notice and the objective documentary counter-record. They still do **not** establish criminal instrumentalisation, prevaricación, fraud, fabrication, corruption, conspiracy, criminal coordination or prosecutorial acceptance of the substantive allegations.

---

## 8. Continuity instruction

Until the older canonical gate/startup wording is textually reconciled, **this addendum controls over any older sentence saying that the signed 27-Jan-2026 Tenerife decree, signed Valencia→Arrecife inhibition, original Plan, signed convalidation decision, 24-Feb-2021 works/condition order or 12-May-2021 re-tender remain wholly missing**.

A future thread must apply stronger/current primary sources over earlier missing-source classifications and must preserve the Annex-5 binary-custody correction.
