# THREAD-DELETION CONTINUITY AUDIT — CALIFICACIÓN / FISCALÍA / 2012–2019–2026 SOURCE BUNDLE

**Date:** 16 August 2026  
**Thread status:** **DELETION-SAFE AT REPOSITORY-KNOWLEDGE LEVEL**  
**Primary merge:** PR #217 — merge `d000f0158af9c2c9324e8e90f9c233f66a75d05f`  
**Pages verification:** build `1155524163` / workflow run `31967682462` — **built/success, no error**, exact merge SHA  
**Continuity target:** fresh ChatGPT must reconstruct the material source intelligence without conversational memory.

## 1. Material intelligence created in the originating thread

The thread ingested ten uploaded PDFs spanning:

- a pre-concurso minority-owner/operator pleading dated 23-Feb-2012;
- DI 248/2018 expansion dated 14-Jan-2019;
- Aweswell's 13-May-2019 opposition to AC escrito 1760/19 and the 7-May decree;
- a 13-Jan-2026 complaint alleging possible judicial prevarication in the AC-separation/supervision track;
- 19-Jan and 2-Feb-2026 banking-origin complaints/expansion, including EG 19/2026;
- 24/25-Jan-2026 unit/Community/insolvency-instrumentalisation complaint and expansion;
- an 8-Feb-2026 expansion of the judicial-prevarication complaint.

The two 24-Jan-2026 autonomous-complaint PDFs are exact binary duplicates and must never be counted as independent corroboration.

## 2. Canonical recovery path

A fresh thread must read:

1. `CHATGPT_START_HERE.md`;
2. `archive/CALIFICACION_FISCALIA_2012_2019_2026_SOURCE_BUNDLE_RETRIEVAL_GATE_16AUG2026.md`;
3. `archive/CALIFICACION_CONCURSO36_PARALLEL_LIVES_PUBLICATION_CONTROL_16AUG2026.md`;
4. `archive/CALIFICACION_DI248_CONTRADICTION_LEDGER_16AUG2026.md`;
5. `archive/FMMM_COMMUNITY_AC_LIQUIDATION_JANFEB2026_SOURCE_BUNDLE_INGEST_AND_RETRIEVAL_GATE_16AUG2026.md`;
6. current `archive/CORRECTION_REGISTER.md`, `archive/MISSING_EVIDENCE_REGISTER.md` and proceedings controls relevant to the question.

The root startup protocol now contains a direct mandatory Rule 20 pointing future threads to this source gate. The gate contains exact filenames, SHA-256 identifiers, source-status classifications, chronology, public-language limits, search aliases and missing-primary-source targets. This direct-path startup rule is controlling even if GitHub code-search indexing lags.

## 3. Assertions a fresh thread must recover correctly

### A. 2012

A 23-Feb-2012 pleading in JV 1260/2011 expands an existing eviction action to Monterecco Sun Park, S.L. after an alleged change in hotel exploitation. This proves pre-concurso owner/operator litigation chronology, not the merits or outcome of that case.

### B. 2019 DI 248

The 14-Jan-2019 DI 248 expansion is contemporaneous evidence that Gil/Aweswell were asking Fiscalía to investigate the AC/CAM perimeter, challenging unitary-asset/OB REM/separate-disposal conduct and asserting a financed Article-176 exit route. It proves notice and allegations, not criminality.

### C. 13-May-2019 unitary-production record

The 13-May Aweswell pleading reproduces the Plan/AC description of CAM's proposal as an `oferta de adquisición unitaria de la unidad productiva` and reproduces the proposition that reception/restaurant/bar/pool/garden common properties were not commercially exploitable independently. This is a high-value adverse-side/source locator but remains subordinate to the original Plan, CAM offer and signed orders/deeds for final legal effect.

### D. 2026 complaints

The Jan–Feb-2026 filings prove that judicial-supervision, unit/Community, insolvency-causation and banking-fragmentation theories were formally raised with prosecutors. Filing does not prove investigation, acceptance, offence or liability.

The 8-Feb pleading reports a 27-Jan-2026 Tenerife decree in Diligencias 20/2026; the signed decree was not part of this source bundle and the report must remain unverified until independently recovered.

## 4. Public-site state actually deployed

PR #217 merged the public runtime module `assets/calificacion-fiscal-2012-2019-2026-bridge-20260816.js` and added its loader to `assets/site.js`.

The module is source-routed to:

- the ES/EN Calificación landing pages, with the full 2012 → 2019 → 2026 documentary bridge;
- the ES/EN Community instrumentalisation pages;
- the ES/EN 7-Jun-2018 takeover pages;
- the ES/EN sale/lender-convergence pages;
- the ES/EN CaixaBank Valencia claim pages;
- the ES/EN institutional-accountability pages;
- the ES/EN judicial-control/prevaricación legal-notebook pages;
- the ES/EN updates pages.

Mandatory reader-facing distinction remains:

> **These documents show allegations, prior notice, contemporaneous objections and the questions formally put to institutions. They do not establish criminal liability.**

No raw private complaint PDF, personal identifier, signature, private address or unnecessary personal data was published by this module.

GitHub Pages built the exact merge commit `d000f0158af9c2c9324e8e90f9c233f66a75d05f` successfully in build `1155524163` / workflow run `31967682462`; the Pages status endpoint reported `built` with no error. External rendered-route retrieval remained unavailable through the current web search/URL-safety path, so deployment verification rests on exact-SHA Pages build plus merged-source/loader verification rather than a browser-rendered DOM capture.

## 5. Evidence custody

The source gate preserves exact SHA-256 identifiers for all reviewed binaries. This thread does **not** claim that all ten have been transferred to the independent private evidence vault. Repository deletion-safety and native evidence-custody resilience remain separate controls.

If native custody is later activated, preserve the originals under stable IDs without modifying the files, hash immediately, record source-native metadata and storage location, and log every custody event.

## 6. Open evidence that must not be forgotten

- certified/full JV 1260/2011 file and outcome;
- native 12-Jul-2016 burofax and annexes;
- complete DI 248/2018 file and annexes;
- original Plan, CAM offer, valuation and 2018 orders;
- 28-Nov-2018 deed/payment, 12-Dec diligence, 9-Jan-2019 Registry qualification, 20-Mar AC escrito 1760/19 and signed convalidation decision;
- filed receipts/file numbers/outcomes for the Jan–Feb-2026 complaints;
- signed 27-Jan-2026 Tenerife Diligencias 20/2026 decree;
- signed Valencia→Arrecife inhibition and complete EG 19/2026 record;
- source-complete Community accounting/maintenance and 2017–18 valuation record.

## 7. Independent fresh-thread verification prompt

A reusable, adversarial verification prompt is preserved at:

`archive/prompts/VERIFY_CALIFICACION_FISCALIA_2012_2019_2026_REPOSITORY_CAPTURE_PROMPT_16AUG2026.md`

It instructs a fresh ChatGPT to ignore conversational memory, open current `main`, verify Rule 20, reconstruct CF-01→CF-10 with hashes and source-status limits, test the 2012→2019→2026 chronology, verify the public runtime/route set and deployment record, and return a PASS / PARTIAL / FAIL matrix with exact repository support. Its purpose is to demonstrate independently that the repository—not this conversation—is carrying the knowledge.

## 8. Final deletion decision

All repository-knowledge continuity gates requested for this thread are now satisfied:

- **canonical source gate merged to `main`: YES**;
- **root `CHATGPT_START_HERE.md` direct-path rule: YES — Rule 20**;
- **bilingual Calificación/public cross-route module merged and loaded: YES**;
- **exact merge SHA deployed by GitHub Pages without error: YES**;
- **deployment state recorded in this canonical thread audit and dedicated deployment evidence: YES**;
- **independent fresh-thread verification prompt preserved in repository: YES**.

Therefore, deletion of the originating ChatGPT conversation will **not** remove the canonical project knowledge created from this source bundle. A fresh Project Sun Rock / Por Derecho thread following `CHATGPT_START_HERE.md` will be routed to the source gate and recover the material source inventory, evidential classifications, corrections, chronology, public implementation, unresolved questions and retrieval targets.

**Remaining boundary:** native evidence-custody resilience for CF-01–CF-10 is separate and must not be inferred from this deletion-safe status.