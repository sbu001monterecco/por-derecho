(() => {
  ...
  const insertJump = () => {
    ...
    if (!nav) return;
    const links = lang === 'es'
      ? [['#recuperados','Recuperados'],['#vidas-paralelas','Vidas paralelas'],['#ricpe','RIC/RICPE'],['#fallos-matriz','Fallos/omisiones']]
      : [['#recovered','Recovered'],['#parallel-lives','Parallel lives'],['#ricpe','RIC/RICPE'],['#failure-matrix','Failures/omissions']];
    const marker = document.createDocumentFragment();
    links.forEach(([href,label]) => {
      if (nav.querySelector(`a[href="${href}"]`)) return;
      const a = document.createElement('a'); a.href=href; a.textContent=label; marker.appendChild(a);
    });
  ...
  const renderFailures = multi => {
    ...
    const sec=document.createElement('section'); sec.className='section'; sec.id='failure-matrix-section';
    sec.innerHTML=`<div class="shell"><span id="${id}"></span><p class="eyebrow">${esc(t.failures)}</p>
      <h2>${esc(tr('No una acusación genérica al juez: cinco pruebas de respuesta y protección','Not a generic accusation against the judge: five response-and-protection tests'))}</h2>
      <p class="lede">${esc(tr('La formulación más fuerte que resiste auditoría separa: resolución judicial adversa; cuestión expresamente puesta ante el Juzgado; respuesta parcial; autoridad no localizada; y seguimiento protector/contable no localizado. La responsabilidad personal sólo puede evaluarse después de cerrar esa cadena.','The strongest audit-resistant formulation separates: adverse judicial ruling; matter expressly put before the court; partial response; authority not located; and protective/accounting follow-up not located. Personal liability can only be assessed after that chain is closed.'))}</p><div class="track-grid">${cards}</div></div>`;
    anchor.insertAdjacentElement('afterend',sec);
  };
  ...
  Promise.all([
    ...
    if (gaps) {
      const merged=[...(matrix.gap_closure||[]),...(multi.gap_closure||[])];
