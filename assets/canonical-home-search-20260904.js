(() => {
  ...
  const existing = document.querySelector('[data-canonical-home-search]');
  if (existing) existing.remove();
  document.querySelectorAll('[data-canonical-home-search-style]').forEach((node) => node.remove());
  ...
  const copy = lang === 'es' ? {
    ...
    placeholder: 'Ej.: Uria Menendez · Juan Francisco Falcon · DP 748/2026 · ^P-0169',
    ...
    placeholder: 'E.g. Uria Menendez · Juan Francisco Falcon · DP 748/2026 · ^P-0169',
  ...
  const collectStrings = (value, output = [], depth = 0) => {
    ...
    if (typeof value === 'string' || typeof value === 'number') output.push(String(value));
      ...
      if (!['url','hash','sha256','email','phone'].includes(key.toLowerCase())) collectStrings(item, output, depth + 1);
    });
    return output;
  };
  ...
  const makeEntry = (record) => {
    ...
    const values = [record.id, name, ...aliases, ...collectStrings(record)];
    ...
    return {
      ...
      route: routeFor(record), norms, exact,
      haystack: norms.join(' | '), collapsed: new Set(norms.map(collapse).filter(Boolean)),
  ...
  const makeMasterEntry = (record) => {
    ...
    return {
      ...
      norms, exact:new Set([record.Master_ID,name,...aliases].map(normalise).filter(Boolean)),
      haystack:norms.join(' | '), collapsed:new Set(norms.map(collapse).filter(Boolean)), tokenSet:new Set(norms.flatMap(tokens)), sourceTier:'MASTER'
    };
  };
