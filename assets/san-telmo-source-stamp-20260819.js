(()=>{
  ...
  const mount=()=>{
    if(d.querySelector('[data-pd-st-source]'))return;
    style();
    ...
    const old=d.querySelector('[data-pd-parallel-lives]');
    if(old)old.insertAdjacentElement('beforebegin',box);
    else{const five=d.querySelector('[data-pd-five-ac]');const hero=d.querySelector('.dossier-hero,.hero,main>section:first-child');const target=five||hero;if(target)target.insertAdjacentElement('afterend',box);else(d.querySelector('main')||d.body).prepend(box)}
