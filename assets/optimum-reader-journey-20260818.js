(() => {
  ...
  const mediaUrl = p(isEn ? 'media-public-narrative-traceability/' : 'medios-trazabilidad-relato-publico/');
  ...
  const addHomeIntent = () => {
    ...
    const section = make(`
      <section class="psr-intent" id="psr-reader-intent" aria-labelledby="psr-reader-intent-title">
        <div class="shell">
          <div class="psr-intent-head"><div><p class="psr-intent-kicker">${t('EMPIECE POR SU FUNCIÓN', 'START WITH YOUR ROLE')}</p><h2 id="psr-reader-intent-title">${t('Un expediente; cuatro puertas de entrada.', 'One record; four ways in.')}</h2><p>${t('No necesita leerlo todo para empezar. Elija la ruta que corresponde a su responsabilidad; cada una vuelve a las mismas fuentes, límites y correcciones.', 'You do not need to read everything to begin. Choose the route matching your responsibility; each returns to the same sources, boundaries and corrections.')}</p></div><aside class="psr-intent-status">${updateStatusHtml()}</aside></div>
          <div class="psr-intent-grid">
            <a class="psr-intent-card" href="${cleanRoomUrl}"><span class="num">01 · ${t('Autoridad pública', 'Public authority')}</span><strong>${t('Resolver por competencia', 'Resolve by competence')}</strong><span>${t('Hechos, aviso, deber, pregunta finita y documento que puede cerrar cada punto.', 'Facts, notice, duty, finite question and the document capable of closing each point.')}</span><em>${t('Abrir sala limpia institucional →', 'Open the institutional clean room →')}</em></a>
            <a class="psr-intent-card" href="${mediaUrl}"><span class="num">02 · ${t('Medio o investigador', 'Journalist or researcher')}</span><strong>${t('Comprobar antes de publicar', 'Verify before publishing')}</strong><span>${t('Cronología, cifras controladas, fuentes primarias, contradicciones y atribución.', 'Chronology, controlled figures, primary sources, contradictions and attribution.')}</span><em>${t('Abrir guía de trazabilidad →', 'Open the traceability guide →')}</em></a>
            <a class="psr-intent-card" href="${recoveryUrl}"><span class="num">03 · ${t('Propietario o afectado', 'Owner or affected party')}</span><strong>${t('Seguir activos, derechos e ingresos', 'Follow assets, rights and income')}</strong><span>${t('Perímetros separados, restitución y vías de aportación documental.', 'Separate perimeters, restitution and routes for responsible evidence.')}</span><em>${t('Abrir objetivos de recuperación →', 'Open recovery objectives →')}</em></a>
            <a class="psr-intent-card" href="${collaborateUrl}"><span class="num">04 · ${t('Profesional o financiador', 'Professional or funder')}</span><strong>${t('Evaluar colaboración responsable', 'Assess responsible collaboration')}</strong><span>${t('Mandato, independencia, conflictos, fondos y evidencia mínima.', 'Mandate, independence, conflicts, funds and minimum evidence.')}</span><em>${t('Abrir criterios de colaboración →', 'Open collaboration criteria →')}</em></a>
          </div>
