...
**Current-main baseline independently checked:** `19fb3b7a5d502ece5f5359cc6cfe4bdc2f858b78`  
...
The broader repository/site audit merged earlier on 25 August 2026 remains controlling for the system-wide digest: `docs/deletion-audits/2026-08-25-unitary-repository-website-valencia-thread-closeout.md`.

That audit found the public system substantially coherent around one recovery-led architecture: separate legal/property/operational perimeters; a source-first judicial and insolvency spine; distinct private-actor, Insolvency Administrator and judicial responsibility lanes; property/title/possession/income separation; funded-exit counterfactual analysis; and recovery allocation without automatic double counting.

Since that redigest, `main` advanced only through the exactly-once outbound-email governance release at `19fb3b7a5d502ece5f5359cc6cfe4bdc2f858b78`. The release added a one-shot send lock, verification allowlist, duplicate-window controls and a public-safe incident record. Publication integrity and the new outbound-email guard passed, and GitHub Pages deployment run `32906752392` completed successfully for that exact SHA.
...
The earlier unitary audit identified legacy public archive/operational material that can expose provider-specific message identifiers, unsent correspondence or private-source derivatives even though current governance forbids such publication. Continue the controlled remediation programme:
...
`ops/CURRENT_STATE.json` is still dated 24 August and still identifies `ed98b0ac...` / PR #922 as the baseline current release while current `main` is `19fb3b7a...`. Its stored open-PR snapshot says 31 while the fresh count is 34.
...
| `LIVE_PUBLICATION_CONTINUITY` | **Safe** — current Pages deployment for `19fb3b7a...` succeeded; prior whole-site audit established exact source/live parity before the governance-only follow-on change |
...
9. `docs/deletion-audits/2026-08-25-unitary-repository-website-valencia-thread-closeout.md` for the unitary system-wide baseline.
