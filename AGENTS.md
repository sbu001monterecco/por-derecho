...
- For UK company number `07716847`, resolve the entity as **AWESWELL LIMITED** before drafting. Before any external email, filing or public post, run `python3 scripts/validate_aweswell_identity.py --outbound <draft-file>`; a forbidden variant is a hard stop unless it is an expressly labelled historical error. `OSWELL 426 S.L.` remains a distinct Spanish entity.
...
- **R33 page-5 continuity lock:** whenever Joaquín's apology/withdrawal is cited, also surface that he ceased acting on 21-Sep-2020, that adverse/removal work concerning the AC pre-dated the withdrawal, and that Joaquín did not carry the route through to the 23-Apr-2025 separation filing. Wire the same proposition to `AC-CLM-011` and `AC-CLM-014`. Preserve the allegation that AC conduct contributed to the discontinuity as an attributed causation/intent hypothesis, not a finding.
- Where DP 1956 is in scope, also expose the page-5 sequence as **cross-evidence only** under `AC-CLM-012`, because Control 22 expressly includes control over LPB's defence/capacity. Do not describe that cross-link as a procedural merger, reopening, amendment or merits finding.
